...
namespace tune {
template <auto ID, typename... T> struct CTunable;

template <auto ID, typename T> struct Tunable;

template <auto ID, alpaka::concepts::Vector T> struct TunableMD;
} // namespace tune
...
template <typename T>
concept ArithmeticComparableOrVec =
    ArithmeticComparable<T> || alpaka::concepts::Vector<T>;
} // namespace tune::concepts

// forward declare Config
namespace tune::config {
template <concepts::Integral T, uint32_t NumTunables> struct Config;
template <concepts::Floating T, uint32_t NumTunables> struct NormalizedConfig;
} // namespace tune::config

namespace tune::concepts {
template <typename T>
concept GpuExecutor =
    std::same_as<std::remove_cvref_t<T>, alpaka::exec::GpuHip> ||
    std::same_as<std::remove_cvref_t<T>, alpaka::exec::GpuCuda> ||
    std::same_as<std::remove_cvref_t<T>, alpaka::exec::OneApi>;
template <typename E>
concept CpuBlocksExec =
    std::same_as<std::remove_cvref_t<E>, alpaka::exec::CpuTbbBlocks> ||
    std::same_as<std::remove_cvref_t<E>, alpaka::exec::CpuOmpBlocks>;

template <typename T> struct isShallowTunableDummy : std::false_type {};

template <auto Tag>
struct isShallowTunableDummy<internal::ShallowTunableDummy<Tag>>
    : std::true_type {};
...
concept runtimeTuneable =
    ...
    && (T::tuneableType == internal::TunableKind::Tunable ||
        T::tuneableType == internal::TunableKind::TunableMD);
// currently compile-time tunables are not permitted
template <typename T>
concept TuneableLike = runtimeTuneable<T> || isShallowTunableDummy_v<T> ||
                       std::is_same_v<T, internal::NoTune>;
template <typename T>
...
concept HasTraitSerializer = requires(T const &v) {
  { ::tune::trait::Serialize<T>{}(v) } -> std::convertible_to<std::string>;
};
 ...
 *  - It provides a trait-based serializer via
 * `serialize::HasTraitSerializer<T>`.
 *  - It defines a `toString()` method returning a `std::string`.
 *  - It supports the stream insertion operator (`operator<<`) for output to
 * `std::ostream`.
 *
 ...
 *  3. Specialize a serializer under `serialize::trait` or `tune::trait`
 *     (e.g., `template<> struct Serialize<MyType> { std::string
 * operator()(MyType const&) const; };`)
 *
 * Example:
 * @code
 * struct MyData {
...
concept Serializable =
    std::is_convertible_v<T, std::string> || std::is_arithmetic_v<T> ||
    serialize::HasTraitSerializer<T> || serialize::HasToStringMethod<T> ||
    serialize::HasStreamOperator<T>;

namespace detail {
template <typename T> struct ConfigLikeHelper {
  using U = std::remove_cvref_t<T>;

  template <typename X> static constexpr bool test(int) {
...
concept KernelTuningModel = requires(T t) {
  ...
  // functions
  // Note: we don’t constrain argument types precisely here, since they’re
  // templated
  {
    t.getValuesFromConfig(std::declval<config::Config<uint32_t, T::numDims>>())
  };
  {
    t.createConfigFromNormalized(
        std::declval<config::NormalizedConfig<double, T::numDims>>())
  };
};

namespace detail {
template <typename... Ts> consteval void checkFrameTupleTypes() {
        ...
        (
            []<typename B>() {
              if constexpr (runtimeTuneable<A> && runtimeTuneable<B>) {
                ...
                static_assert(std::is_convertible_v<typename A::value_type,
                                                    typename B::value_type> ||
                                  std::is_convertible_v<typename B::value_type,
                                                        typename A::value_type>,
                              "All frame tuneables must have mutually "
                              "convertible ::value_type");
              }
...
template <typename Tuple> struct isValidFrameTupleImpl : std::false_type {};

template <typename... Ts> struct isValidFrameTupleImpl<std::tuple<Ts...>> {
...
struct isValidFrameTuple<Tuple, std::void_t<>>
    : std::bool_constant<detail::isValidFrameTupleImpl<Tuple>::value> {};
