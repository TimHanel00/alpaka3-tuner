...
namespace tune::strategy {
namespace helper {
template <concepts::Integral T, auto N>
inline void vectorFromConfig(std::vector<T> &vec,
                             config::Config<T, N> const &config) {
  if (vec.size() != N)
    vec.resize(N);
...
inline void
vectorFromNormalizedConfig(std::vector<T> &vec,
