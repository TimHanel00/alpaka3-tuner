...
template <typename Tuple, typename F> void for_each(Tuple &&tup, F &&f) {
  ...
  for_each_impl(std::forward<Tuple>(tup), std::forward<F>(f),
                std::make_index_sequence<N>{});
}

template <typename Tuple, typename F, std::size_t... Is>
constexpr void for_each_enumerate_constexpr_impl(Tuple &&tup, F &&f,
...
// Entry point
template <typename Tuple, typename F, std::size_t... Is>
constexpr void for_each_enumerate_runtime_impl(Tuple &&tup, F &&f,
                                               std::index_sequence<Is...>) {
  (f(std::get<Is>(tup), Is), ...);
}
...
constexpr void for_each_enumerate(Tuple &&tup, F &&f) {
  if constexpr (requires { f.template operator()<0>(std::get<0>(tup)); }) {
    constexpr std::size_t N = std::tuple_size_v<std::remove_reference_t<Tuple>>;
    for_each_enumerate_constexpr_impl(std::forward<Tuple>(tup),
                                      std::forward<F>(f),
                                      std::make_index_sequence<N>{});
  } else {
    constexpr std::size_t N = std::tuple_size_v<std::remove_reference_t<Tuple>>;
    for_each_enumerate_runtime_impl(std::forward<Tuple>(tup),
                                    std::forward<F>(f),
                                    std::make_index_sequence<N>{});
  }
}
...
auto visitIndexVariant(std::size_t i, Tuneable const &t) {
  using VariantT =
      decltype(make_variant_type<Tuneable>(std::make_index_sequence<N>{}));
  VariantT result;
  bool _found = false;
