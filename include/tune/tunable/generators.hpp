...
std::vector<T> generateSequence(T start, T end, T step, F &&f) {
  std::vector<T> values;
  for (T v = start; tune::internal::utils::allTrue(v <= end); f(v, step)) {
    values.push_back(v);
  }
  return values;
}
...
struct c_LinSpace {
  ...
  static constexpr auto generate_impl() {
    if constexpr (Cur > End)
      ...
      return generate_impl<Cur + Step, CVecs...,
...
struct c_LogSpace {
  ...
  static constexpr auto generate_impl() {
    if constexpr (Cur > End)
      ...
      return generate_impl<Cur * Base, CVecs...,
                           std::integral_constant<T, Cur>>();
  }
