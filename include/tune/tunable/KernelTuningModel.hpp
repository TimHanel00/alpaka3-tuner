...
template <typename Tuple> constexpr auto makeOffsets() {
  return makeOffsetsImpl<Tuple>(
      std::make_index_sequence<std::tuple_size_v<Tuple>>{});
}
} // namespace internal

/**
 * @brief A ParameterAccessor to provide the concrete value of a tunable
 * parameter. It also contains information about the name, ID, and kind of the
 * corresponding tunable object.
 *
 * @tparam T_Value     The underlying value type of the parameter.
 * @tparam TuneableId  A compile-time constant identifier associated with the
 * tunable parameter.
 * @tparam TuneableKind The kind of tunable parameter
 * (e.g.,Tuneable,TuneableMD,CTuneable), represented by tune::TuneableKind.
 *
 * @details
 * Each ParameterAccessor instance acts as a lightweight view on an existing
 * tunable configuration value. It does not own the value, but holds a constant
 * reference to it together with a reference to its name.
 */
template <typename T_Value, std::size_t TuneableId,
          internal::TunableKind TuneableKind>
...
template <typename T_KernelTuningModel> struct ConfigDescriptor {
  explicit ConfigDescriptor(T_KernelTuningModel const &tuningModel)
      : m_kernelTuningModel(tuningModel) {};

  /**
   * @brief Construct a tuple of ParameterAccessors from a parameter
   * configuration (Config). A parameterAccessor contains the Values of the
   * tuneable parameter corresponding to this Config alongside metadata (name,
   * ID, kind). For Ctuneables the value is not returned directly.
   *
   * @param config The parameter configuration.
   * @tparam idx_type The idx_type of the parameter configuration (should be
   * automatically deducible).
   * @tparam NumTuneables The number of Tuneables of the parameter configuration
  ...
  /**
   * @brief turns a normalized Configuration (floating based) into a integer
   * based configuration*/
  ...
  /**
   * @brief turns a integer based Configuration into a floating point based
   ...
   * as a integer based parameter configuration
   *
   *
...
auto makeAllTunables(T_FrameTuple &frame_tuple, T_UserTuple &user_tuple,
                     T_CompileTuple &compile_tuple) {
  return std::tuple_cat(
      std::apply([](auto &...elems) { return std::tie(elems...); },
                 frame_tuple),
...
template <typename T_FrameTunables = std::tuple<>,
...
struct KernelTuningModel {
  ...
  using T_allTunablesBare =
      alpaka::meta::Concatenate<T_FrameTunables, T_UserTuple,
                                T_CompileTimeTuple>;
  using T_allTunablesRef = decltype(makeAllTunables(
      m_frameTuneables, m_userTuneables, m_compileTimeTuneables));
  ...
  constexpr KernelTuningModel(T_FrameTunables frameT, T_UserTuple userT,
                              T_CompileTimeTuple compileT)
      : m_frameTuneables(std::move(frameT)), m_userTuneables(std::move(userT)),
        m_compileTimeTuneables(std::move(compileT)),
        m_allTunables(makeAllTunables(m_frameTuneables, m_userTuneables,
                                      m_compileTimeTuneables)),
        m_numValues(getNumValues()) {}

  // this custom move constructor ensures we rebuild references stored in
  // m_allTuneables (including references) after a move
  ...
  template <concepts::Floating floating_type, auto NumTuneables>
  constexpr auto createConfigFromNormalized(
      tune::config::NormalizedConfig<floating_type, NumTuneables> const &config)
      const {
    static_assert(
    ...
    for (std::size_t val = 0; val < NumTuneables; ++val) {
      auto const scaled =
          std::floor(config[val] * static_cast<double>(m_numValues[val]));
    ...
    return tune::config::Config{return_config_ar};
  }

  /**
   * @brief Turns an integer-based configuration into a normalized (floating)
   * configuration.
   */
  template <typename idx_type, auto NumTuneables>
  constexpr auto createNormalizedFromConfig(
      tune::config::Config<idx_type, NumTuneables> const &config) const {
    static_assert(NumTuneables == numDims,
                  " Config does not have the correct number of parameters! ");

    std::array<double, NumTuneables> normalized_values{};

    for (std::size_t val = 0; val < NumTuneables; ++val) {
      normalized_values[val] = static_cast<double>(config[val]) /
                               static_cast<double>(m_numValues[val]);
    ...
    return tune::config::NormalizedConfig<double, NumTuneables>{
        normalized_values};
  }

  /**
   * @brief returns the indicies corresponding to the compile-time tuneable
   *indicies of the parameter configuration as a
   *std::array<idx_type,std::tuple_size_v<T_CompileTimeTuple>>
   *
   * @param config The parameter configuration.
   * @tparam idx_type The idx_type of the parameter configuration (should be
   *automatically deducible).
  ...
  getConfigSubset_CompileTuneables(
    ...
    for (std::size_t i = offsets[startingIdx], a = 0;
         i < NumTuneables && a < ret.size(); ++i, a++) {
      ret[a] = config[i];
    }
    return ret;
  }
   ...
   * @brief Construct a tuple of ParameterAccessors from a parameter
   * configuration (Config). A parameterAccessor contains the Values of the
   * tuneable parameter corresponding to this Config alongside metadata (m_name,
   * ID, kind) as a std::tuple. More concretely member m_value contains the
   * values of the produced configuration for Tuneable and TuneableMD. The
   * values for the compile-time tuneable have can be accessed using std::visit:
  ...
  constexpr auto getValuesFromConfig(
    ...
    return getValuesFromTuple<startingIdx, idx_type, NumTuneables,
                              T_allTunablesRef>(config, m_allTunables);
  }
  ...
  auto getNumValues() const {
    ...
    meta::for_each(m_allTunables, [&]<typename T0>(T0 const &elem) {
      using ElemT = std::decay_t<T0>;

      auto vec = elem.getNumValues(); // runtime values per dimension

      for (uint32_t dim = 0; dim < alpaka::getDim(vec) && curIdx < numDims;
           ++dim, ++curIdx) {
  ...
  auto getInitConfig() {
    ...
    auto idx = 0;
    meta::for_each(m_allTunables, [&](auto const &elem) {
  ...
  [[nodiscard]] uint32_t getMaxPossibleRuns() const {
    ...
    for (uint32_t i = 0; i < m_numValues.size(); i++) {
      ...
      if (tmp < maxPossibleRuns) {
        std::cerr << " overflow during tuning space calculation, this error "
                     "can be ignored if the tuning "
                     "space is restricted by environment variables "
                     "TunerMaxConfigEvaluations or "
   ...
   * @brief Construct a tuple of ParameterAccessors from a parameter
   * configuration (Config). A parameterAccessor contains the Values of the
   * tuneable parameter corresponding to the runtime-Tuneable subset of this
   * Config alongside metadata (m_name, ID, kind) as a std::tuple. More
   * concretely member m_value contains the user defined runtime-tunables values
   * corresponding to this parameter configuration.
   *
  ...
  [[nodiscard]] constexpr auto getValuesForRuntimeTuneables(
    ...
    return getValuesFromTuple<startingIdx, idx_type, NumTuneables, T_UserTuple>(
        config, m_userTuneables);
  }

  /**
   * @brief Construct a tuple of ParameterAccessors from a parameter
   * configuration (Config). A parameterAccessor contains the Values of the
   * tuneable parameter corresponding to the compile-time-Tuneable subset of
   * this Config alongside metadata (m_name, ID, kind) as a std::tuple. More
   * concretely member m_value contains the the compileTime-tunables values
   * corresponding to this parameter configuration as a std::variant. It can be
   * access using std::visit([](auto&& v) {  },
   * std::get<I>(parameterAccessor).m_value); where index: I -> corresponds to
   * the I`th compile-time parameter index.
   *
   * @param config The (whole) parameter configuration.
   * @tparam idx_type The idx_type of the parameter configuration (should be
   * automatically deducible).
   ...
   * @brief Construct a tuple of ParameterAccessors from a parameter
   * configuration (Config). A parameterAccessor contains the Values of the
   * tuneable parameter corresponding to the frameSec-Tuneable subset of this
   * Config alongside metadata (m_name, ID, kind) as a std::tuple. The member
   * m_value contains the frameSpec-tuneable values corresponding to this
  ...
  [[nodiscard]] constexpr auto getValuesForFrameTuneables(
    ...
    return getValuesFromTuple<startingIdx, idx_type, NumTuneables,
                              T_FrameTunables>(config, m_frameTuneables);
  }
   ...
   */
  template <
      alpaka::concepts::Vector T_Frames, alpaka::concepts::Vector T_Extent,
      alpaka::concepts::Vector T_Blocks, typename idx_type, auto NumTuneables>
  void applyToFrameSpec(
    ...
    if constexpr (size != 0) {
      meta::for_each_enumerate(
          frameTunes, [&]<std::size_t I, typename T0>(T0 const &tuneable) {
  ...
  template <typename Tuple, auto ID> static constexpr bool hasTuneableTag() {
    return hasTagInTuple<Tuple, ID>();
  }

  template <auto ID> static constexpr bool hasFrameTuneable() {
    constexpr auto ID_v = static_cast<std::size_t>(ID);
    return hasTuneableTag<T_FrameTunables, ID_v>();
  }

  template <auto ID> static constexpr bool hasUserTuneable() {
    constexpr auto ID_v = static_cast<std::size_t>(ID);
    return hasTuneableTag<T_UserTuple, ID_v>();
  }

  template <auto ID> static constexpr bool hasTuneable() {
  ...
  template <auto... ID> constexpr auto getByIDs() {
    return std::make_tuple(getByID<ID>()...); // expands each getByID
  }

  template <auto ID> constexpr auto *getByID() {
    constexpr auto ID_v = static_cast<std::size_t>(ID);
...
private:
   ...
   */
  template <auto startingIndex, typename idx_type, auto NumTuneables,
            typename SubsetTuple>
  constexpr auto
  getValuesFromTuple(tune::config::Config<idx_type, NumTuneables> const &config,
                     SubsetTuple const &tuneablesSubset) const {
    static_assert(NumTuneables == numDims,
                  " Config has the wrong number of parameters!");
    ...
    auto handleTuneable = [&]<typename T0>(T0 const &tuneable, auto I_c) {
      ...
      constexpr std::size_t fullIndex =
          startingIndex + I_c; // correct slice in the global tuple
      ...
      if constexpr (kind == internal::TunableKind::CTunable) {
        auto valueVariant =
            meta::visitIndexVariant<std::tuple_size_v<typename T0::Values>>(
                config[offset],
                tuneable); // variant use std::visit for runtime dispatch

        return std::tuple{ParameterAccessor<decltype(valueVariant), ID, kind>{
            valueVariant, tuneable.getName()}};
      } else {
        if constexpr (kind == internal::TunableKind::TunableMD) {
          ...
          return std::tuple{ParameterAccessor<decltype(value), ID, kind>{
              value, tuneable.getName()}};
        } else {
          if constexpr (kind == internal::TunableKind::Tunable) {
            ...
            auto const &value =
                tuneable.getValueByIndex(vec); // call by reference
            return std::tuple{ParameterAccessor<decltype(value), ID, kind>{
                value, tuneable.getName()}};
          }
        }
    ...
    auto outside_tuple = [&]<std::size_t... I>(std::index_sequence<I...>) {
      return std::tuple_cat(
          handleTuneable(std::get<I>(tuneablesSubset),
