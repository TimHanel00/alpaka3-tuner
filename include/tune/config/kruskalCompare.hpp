...
namespace tune::internal::config {
/// \brief Ternary outcome for statistical comparison between two configurations
/// used to allow premature termination of configuration.
...
/**
 * @brief Non-parametric comparison (Kruskal–Wallis, df=1) between two measured
 * configs.
 *
 * Pools and ranks samples from both records. If H < 3.841 (α=0.05), the result
 * is Inconclusive. Otherwise returns Less/Greater based on medians. Invalid if
 * @p other is marked Invalid.
 *
...
inline Comparison
kruskalCompare(tune::config::ConfigRecord<T_Config> const &current,
  ...
  double_t H =
      (12.0 / (N * (N + 1))) * (R0 * R0 / n0 + R1 * R1 / n1) - 3 * (N + 1);

  if (constexpr double_t chiSquareCritical = 3.841; H < chiSquareCritical)
    return Comparison::Inconclusive;

  double_t lhsMedian =
      current.getMeasurements().get(median_t{}).template as<t_ns>();
  double_t rhsMedian =
      other.getMeasurements().get(median_t{}).template as<t_ns>();
