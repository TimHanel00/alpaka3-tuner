...
enum class Comparison { Less, Greater, Inconclusive };
...
template <typename T> struct metricWrapper {
  T value;

  template <typename Unit> [[nodiscard]] double_t as() const {
 ...
 *
 * Also handles confidence interval (CI) checks to support the local break
 * criteria logic (configEntry is finished) (default: 99% CI within 5% of
 * median).
 *
...
public:
  ...
  bool push(double_t val, bool checkCI = true) {
    ...
    meanVal = (meanVal * static_cast<double_t>(count) + val) /
  ...
  [[nodiscard]] std::span<double_t const> getAll() const { return history; }

  std::size_t size() const { return history.size(); }

  [[nodiscard]] bool empty() const { return history.empty(); }

  [[nodiscard]] metricWrapper<double_t> get(min_t) const { return {minVal}; }

  [[nodiscard]] metricWrapper<double_t> get(max_t) const { return {maxVal}; }

  [[nodiscard]] metricWrapper<double_t> get(mean_t) const { return {meanVal}; }
  ...
  /**
   * this is an expensive operation which shouldnt be used too frequently, it
   * rebuilds the priority queues from scratch after pop
  ...
  /**
   * after every k (where k <=> stepsUntilCICheck) steps we perform a check if
   * the 99% Confidence Intervall (indicating 99% certainty that the true
   *
   * median is contained in that range) deviates less then 5% from the
   * detected/observed median
   * */
  bool ciWithinTolerance(double_t zscore = 2.576 /*z score for 99% CI */,
    ...
    int lowerIdx =
        std::max(0, static_cast<int>(std::floor((n - z * std::sqrt(n)) / 2)));
    int upperIdx =
        std::min(static_cast<int>(n - 1),
                 static_cast<int>(std::ceil((1 + (n + z * std::sqrt(n)) / 2))));
...
private:
  ...
  std::priority_queue<double_t, std::vector<double_t>, std::greater<>>
      upper; // min-heap
  ...
  void rebuildFromHistory() {
    ...
    for (auto const &val : history) {
      ...
      meanVal =
          (meanVal * static_cast<double_t>(lower.size() + upper.size()) + val) /
...
constexpr bool is_stat_type_v =
    std::is_same_v<T, min_t> || std::is_same_v<T, max_t> ||
    std::is_same_v<T, mean_t> || std::is_same_v<T, median_t>;
} // namespace tune::internal
