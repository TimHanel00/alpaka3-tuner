...
namespace tune::store {
/**
 * @brief Runtime history (active window) of all measured configurations.
 *
 * `ActiveHistory` is a session-local database that records each tested
 * configuration and its corresponding measurement record
 * (`ConfigRecord<TConfig>`). It allows:
 *   - O(1) lookup and update by configuration key.
 *   - Sequential replay of configurations in insertion order.
 *
 * This history is **append-only**: once a configuration is added, it remains
 * valid until the end of the tuning session. This guarantees consistency of
 * references across other tuning subsystems (such as `EnvironmentState` or user
 * strategies).
 *
 * @tparam TConfig
 ...
 *
 * @note
 *  - Configurations MUST NEVER be removed.
 *  - The RuntimeHistory must outlive the queue (as it is storing references)
 *  - The map and ordered list share ownership of the same `Entry` instances.
 *  - Thread-safety is not guaranteed; synchronization must be done externally
 * if needed.
 */
template <typename TConfig> struct RuntimeHistory {
  /// @brief Alias for stored entry type (configuration + metrics).
  using Entry = config::ConfigRecord<TConfig>;

  /**
   * @brief Insert or lookup an entry (move overload).
   *
   * If the configuration is new, a new entry is created and tracked in
   * insertion order. Otherwise, returns a reference to the existing one.
   *
  ...
  /// \return Optional reference to the stored entry, or std::nullopt if not
  /// found.
  [[nodiscard]] std::optional<std::reference_wrapper<Entry>>
  getRecord(TConfig const &config) noexcept {
  ...
  [[nodiscard]] std::unordered_map<TConfig, Entry> const &
  getAll() const noexcept {
    return entries;
  }

  /// @brief Access the configurations in insertion order.
  [[nodiscard]] std::vector<std::reference_wrapper<Entry>> const &
  getOrderedHistory() const noexcept {
