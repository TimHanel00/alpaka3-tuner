 ...
 */
#pragma once
#include "tune/config/updateMetric.hpp"
#include "tune/core/StrategyContext.hpp"
#include "tune/traits/traits.hpp"
#include "tune/utils/compileTimeTemplates.hpp"
#include "tune/utils/transformKernelBundle.hpp"
#include <alpaka/onHost/interface.hpp>
namespace tune::internal::core {
/**
 * @brief Runtime driver for a tuning context.
 *
 * Owns the main control loop of the Tuning Process: creates next configs via
 * the strategy, enforces constraints, enqeues and executes alpaka kernels,
 * instruments kernel calls using the MetricInterface, updates history/state,
 * and persists results.
 *
 * @tparam EnvBase A concrete TuningContext (provides model, history, strategy,
 * metric, break criteria).
 */
template <typename EnvBase> class TuningContextManager : public EnvBase {
public:
  using Base = EnvBase;
   ...
   * It decides whether to execute the best configuration -- or while still in
   * the tuning process this function may: Execute the strategy. Evaluate
   * Constraints. Insert configs into the queue. Fetch parameter configuration
   * from the queue. Execute alpaka kernels with a discrete parameter
   * configuration. Evaluate local break criteria (decide wether a single
   * parameter configuration can be dropped) Evaluate global break criteria
   * (indicates this particular tuning Context is finished).
   *
   * @param launchArgs
   *         Forwarded arguments for kernel launch (e.g., queue, exec,
   * KernelBundle{}, kernelargs...).
   *
   ...
   *       transition between exploration, exploitation, and finalization
   * phases.
   *
   ...
   * @see handleFullQueue()
   */
  template <typename... T_Args> void launch(T_Args &&...launchArgs) {
    ...
    this->env_config_queue.try_insert(
        entry.value().get()); // insert valid + new config entry to queue
    emptyTheQueue(std::forward<T_Args>(launchArgs)...);
  }

private:
  /**
   * @brief Drain one config from the queue, execute, update metrics; finalize
   * if empty.
   *
   * Writes persistent history exactly once before marking the session as
   * finished.
   *
   * @tparam T_Args Launch argument types.
   * @param launchArgs Forwarded to kernel execution.
   */
  template <typename... T_Args> void emptyTheQueue(T_Args &&...launchArgs) {
    ...
    std::cout << " filename is empty: "
              << this->env_persistentHistory.m_filename.empty() << std::endl;
  ...
  /**
   * @brief Execute from queue when full or over budget (maxConfigsTotal);
   ...
   */
  template <typename... T_Args> bool handleFullQueue(T_Args &&...launchArgs) {
    if (this->env_config_queue.full() ||
        this->env_config_queue.size() >=
  ...
  /**
   * @brief Run the best-known configuration (used in the lasting final phase of
   * the tuning process).
   *
   * @tparam T_Args Launch argument types.
   * @param launchArgs Forwarded to kernel execution.
   ...
   */
  template <typename... T_Args> void executeBestConfig(T_Args &&...launchArgs) {
    auto const &bestConfig = this->env_environmentState.getBestConfig();
    if (bestConfig.has_value()) {
      applyAndExecute(std::forward<T_Args>(launchArgs)...,
                      bestConfig.value().get());
    } else {
      throw std::runtime_error(
          "Trying to run a best configuration, without any prior tuning runs!");
    }
  }

  /**
   * @brief Execute the Kernel for a given parameter configuration
   * (alpaka::enqueue + wait()). Materialize a concrete frame spec and kernel
   * bundle from the selected configRecord. Dispatches the corresponding
   * compile-time variants of the Kernel. Surrounds alpaka::enqueue +
   * queue.wait() with the metric interface, and returns the measured metric
   * value.
   *
   * @Note Kernel execution is ensured by explicitly synchronizing (with
   * wait()).
   *
   ...
   */
  template <typename T_Queue, typename T_Exec, typename T_FrameSpecTuningModel,
            typename T_Kernelbundle, typename T_Config>
  double_t
  applyAndExecute(T_Queue const &queue, T_Exec &&exec,
                  T_FrameSpecTuningModel const &specModel,
                  T_Kernelbundle const &kernelbundle,
                  tune::config::ConfigRecord<T_Config> const &configRecord) {
    static auto currentSpec = alpaka::onHost::FrameSpec{
        specModel.m_spec.m_numFrames, specModel.m_spec.m_frameExtent,
    ...
    auto argsFromUserTunables =
        this->env_tuningModel.getValuesForRuntimeTuneables(
            configRecord.m_config);
    auto bundle = internal::recreate(kernelbundle, argsFromUserTunables);

    trait::callPreProcessing(this->env_tuningModel, currentSpec,
                             this->env_metricInterface, bundle);
    ...
    if constexpr (!trait::hasUserDefinedCTuneable<KernelFn>::value) {
      ...
      auto indicies = this->env_tuningModel.getConfigSubset_CompileTuneables(
          configRecord.m_config);
      compileTimeHelpers::runtime_Kernel_dispatch<KernelFn>(
          indicies, [&](auto &&element) {
            auto newBundle = alpaka::apply(
                [&element]<typename... T0>(T0 &&...args) {
                  return alpaka::KernelBundle{element,
                                              std::forward<T0>(args)...};
                },
                bundle.m_args);
    ...
    trait::callPostProcessing(this->env_tuningModel, currentSpec,
                              this->env_metricInterface, bundle);
    return metric;
  }

  /**
   * @brief Ask the strategy for the next candidate config; ensure validity and
   * constraints.
   *
   * Converts normalized outputs to discrete configs if needed, enforces
   * strategy break criteria and user constraints, and returns a discrete
  ...
  [[nodiscard]] auto selectNextConfig(uint32_t numAttempts = 0) {
    ...
    auto ctx = tune::StrategyContext<decltype(this->env_tuningModel),
                                     decltype(this->env_metricInterface)>{
        view, this->getHistory(), this->env_environmentState};

    // Run strategy
    auto config = this->env_strategy(ctx);
    using configType = decltype(config);
    static_assert(std::is_convertible_v<T_Normalized, configType> ||
                      std::is_convertible_v<T_Config, configType>,
                  "Strategy has to return a Config!");

    if constexpr (std::is_same_v<std::remove_cvref_t<T_Config>,
                                 std::remove_cvref_t<configType>>) {
      currentConfig = std::move(config);
    } else {
      currentConfig =
          std::move(this->env_tuningModel.createConfigFromNormalized(config));
    }
    ...
    tune::config::ConfigRecord<decltype(currentConfig)> &configRecord =
  ...
  /**
   * @brief Update metrics/history/environment for a just-executed
   * configuration.
   *
  ...
  void update(tune::config::ConfigRecord<T_Config> &stored,
              double_t const &metric) {
    static constexpr bool skip =
        ALLOW_PREMATURE_CONFIG_SKIP; //@TODO get this from the context
    config::updateMetrics<skip, typename Base::T_MetricInterfaceType, T_Config>(
        stored, this->env_environmentState, metric);
  }
};
