...
#include "tune/core/TuningContext.hpp"
...
auto *setup_enqueue(T_Queue const &queue, T_Exec const &exec,
...
template <typename T_Strategy = tune::strategy::RandomSearch,
          concepts::MetricInterface T_MetricInterface =
              tune::metricInterface::Timing,
...
struct TuningSession {
  ...
  enqueue(T_Queue const &queue, alpaka::concepts::Executor auto const &exec,
    ...
    return enqueue_impl(queue, exec,
...
private:
  ...
  auto enqueue_impl(T_Queue &queue, alpaka::concepts::Executor auto const &exec,
                    T_FrameSpec &&spec,
                    alpaka::concepts::KernelBundle auto const &kernelBundle) {
