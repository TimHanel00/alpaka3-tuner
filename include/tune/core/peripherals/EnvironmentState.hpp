...
namespace tune::core::peripherals {
#define Tuner_MaxConsecutiveStrategyFailures 20000

/**
 * @brief Tracks the current tuning session state and global termination
 * criteria.
 *
 * The `EnvironmentState` structure acts as the runtime environment
 * for the tuning process, recording how many configurations have been
 * evaluated, whether global or strategy-specific break conditions have been
 * met, and which configuration currently represents the best-performing
 * candidate.
 *
 * @note
 ...
 * Once these evaluate to true, the tuning loop clears the config Queue or runs
...
template <typename T_Config> struct EnvironmentState {
   ...
   * candidates without full results and promotes entries that have collected
   * measurements over those that do not.
   *
  ...
  void updateBestConfig(config::ConfigRecord<T_Config> const &config_entry) {
    ...
    if (bestConfig.has_value()) {
      ...
      // A) Current best has no metrics, new candidate does -> promote
      // immediately.
      if (before.getMeasurements().empty() &&
          !config_entry.getMeasurements().empty()) {
        bestConfig.emplace(std::ref(config_entry));
        return;
      }

      // B) New candidate isn't complete or is invalid -> ignore it.
      if (config_entry.state != internal::config::ConfigState::Retired)
        return;

      // C) Both comparable -> keep the better one according to the metric
      // interface.
