 ...
 */
#pragma once
#include "tune/adjust/api.hpp"
#include "tune/core/TuningContextManager.hpp"
#include "tune/core/peripherals/ConfigQueue.hpp"
#include "tune/core/peripherals/EnvironmentState.hpp"
#include "tune/interfaces/environmentVars.hpp"
#include "tune/store/KernelTuningMetadata.hpp"
#include "tune/store/PersistentHistory.hpp"
#include "tune/store/RuntimeHistory.hpp"
#include "tune/tunable/KernelTuningModel.hpp"
...
template <bool B, class U> constexpr auto tuple_if(U &&u) {
  if constexpr (B)
...
template <typename T> constexpr auto specToFrameTupleHelper(T const &t) {
 ...
 * @brief Runtime container that owns all state and peripherals to manage a
 * tuningProcess's lifecycle
 */
...
public:
  ...
  tune::store::RuntimeHistory<T_Config> env_activeHistory;
  /// @brief Static metadata describing this tuning context
  /// (executor,device,specifiers...)
  store::KernelTuningMetadata env_metaData;
  /// @brief Global counters/limits (budget, best-so-far, sessionfinish flags).
  tune::core::peripherals::EnvironmentState<T_Config> env_environmentState;
  /// @brief Pending configurations to evaluate (queue).
  peripherals::ConfigQueue<tune::config::ConfigRecord<T_Config>>
      env_config_queue;
  ...
  store::PersistentHistory &env_persistentHistory;

  auto &getHistory() { return env_activeHistory; }
   ...
   * Lazy validation: If violated, marks the configRecord invalid and ensures it
   * will not be scheduled.
   *
   * \param configRecord The configRecord to validate and (if needed)
   * invalidate.
   * \return true if the configuration is invalid or was made invalid, false
   * otherwise.
   */
  bool violatesConstraint(tune::config::ConfigRecord<T_Config> &configRecord) {
    if (configRecord.state == config::ConfigState::Invalid) {
      return true;
    }
    auto parameterAccessor =
        this->env_tuningModel.getValuesFromConfig(configRecord.m_config);
    bool valid = true;
    meta::for_each(this->env_constraints, [&](auto &constraint) {
    ...
    if (!valid) {
      ...
      configRecord.nr_runs =
          std::numeric_limits<decltype(configRecord.nr_runs)>::max();
      return true;
    }
  ...
  /**
   * \brief Construct a tuning context, optionally seeding from persistent
   * history.
   *
   * Loads persistent history (if a filename is provided), clamps budget by
   * environment limits, seeds the initial configuration, and enqueues it if
   * valid.
   *
   * \param env_metadata      Session metadata (device, exec, kernel id, metric,
   * tags).
   ...
   * \param filename          Persistent history filepath (empty disables
   * persistence).
   */
  TuningContext(store::KernelTuningMetadata &&env_metadata, T_FrameSpec,
                T_Strategy strategy_, T_MetricInterface metric_interface_,
                T_Constraints constraints_,
                T_KernelTuningModel &&kernel_tuning_model_,
                std::string const &filename)
      : env_metaData(std::forward<store::KernelTuningMetadata>(env_metadata)),
        env_strategy(std::move(strategy_)),
        env_metricInterface(std::move(metric_interface_)),
        env_constraints(std::move(constraints_)),
        env_tuningModel(
            std::forward<T_KernelTuningModel>(kernel_tuning_model_)),
        env_persistentHistory(store::PersistentHistory::get(filename))

  {
    ...
    if (env_environmentState.getBestConfig().has_value()) {
      ...
      auto mutableBestConfig = getHistory().getOrCreate(
          env_environmentState.getBestConfig().value().get().m_config);
    ...
    tune::config::ConfigRecord<T_Config> &firstEntry =
        getHistory().getOrCreate(initConfigTmp);
 ...
 *
 * Adjusts the frame-spec for the current HW, extracts compile-time and user
 * tunables, builds the kernel tuning model, and returns a context manager
 * owning the TuningContext.
 *
 * \return std::unique_ptr to a context manager for the constructed
 * TuningContext.
 */
template <typename T_Device, typename T_Exec, typename T_FrameSpecTune,
          typename T_KernelBundle, typename... T_Args>
auto createTuningContext(T_Device device, T_Exec exec,
                         T_FrameSpecTune const &spec, T_KernelBundle bundle,
                         TuningSession<T_Args...> const &session) {
  // Apply HW-specific constraints and adjust frameSpec Tunings -- also includes
  // the mechanism to automatically deciding on a parameter space
  auto newFrameSpecTune = adjust::adjustFrameSpecTune(device, exec, spec);
  // Extract compile-time tuneables for bundle
  auto CTuneableBundle = compileTimeHelpers::getCTunables<
      typename std::remove_cvref_t<T_KernelBundle>::KernelFn>();
  // extract User Tunable from ther KernelBundle
  auto userTuple = internal::extractTuneables(bundle);
  // Combine all (frame, user and compileTimeTunables into the
  // KernelTuningModel)
  auto completeTuningModel =
      KernelTuningModel{detail::specToFrameTupleHelper(newFrameSpecTune),
                        userTuple, CTuneableBundle};
  using T_completeTuningModel = decltype(completeTuningModel);

  // @TODO shrinkTuningSpace(completeRun.allTuneables(),
  // completeRun.getNumValues()*);
  using T_MetricType = typename TuningSession<T_Args...>::MetricType;
  // Build metadata
  ...
  using T_Config =
      tune::config::Config<uint32_t, T_completeTuningModel::numDims>;
  ...
  return std::make_unique<T_Context>(
      std::move(env_kernelData), newFrameSpecTune.m_spec, session.m_strategy,
      session.m_metricInterface, session.m_constraintTuple,
      std::move(completeTuningModel), session.m_outputFile);
}
 ...
 */
inline std::string
flattenSessionSpecifier(std::vector<std::string> const &vec) {
  std::string result;
 ...
 *
 * Uses a per-process singleton map keyed by session specifiers; the same key
 * yields the same tuningContext pointer, allowing multiple call sites to reuse
 * the tuning state.
 *
 ...
 * \param session          Tuning session carrying strategy, metricInterface,
 * constraints.
 * \return Reference to a unique_ptr owning the environment (stable across calls
 * with same key).
 */
template <typename T_Queue, typename T_Exec, typename T_FrameSpecTune,
          typename T_KernelBundle, typename... T_Args>
auto &getTuningEnvironment(T_Queue const &queue, T_Exec const &exec,
