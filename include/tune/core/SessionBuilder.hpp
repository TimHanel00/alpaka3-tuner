 ...
 */
#pragma once
#include "tune/core/peripherals/Constraint.hpp"
#include "tune/interfaces/metricInterface.hpp"
#include "tune/interfaces/strategy.hpp"
#include "tune/utils/processVariadicArgs.hpp"
#include <string>
#include <tuple>
 ...
 */
static auto getName() { return strat_name; }
} // namespace strategy::detail
 ...
 *
 * The `TuningBuilder` class provides a configurable interface to define options
 * for the Alpaka Tuner. It acts as a factory for creating a `TuningSession`,
 * which can be used to perform the actual parameter search and evaluation.
 *
 * The builder allows the user to:
 *  - Select a tuning **strategy** (e.g., exhaustive, random search, Bayesian
 * optimization)
 *  - Choose a **metric interface** (default: @ref metricInterface::Timing)
 *  - Define **constraints** between tunable parameters
 *  - Set **session specifiers**, which define distinct tuning contexts (in
 * addition to the default context, which consists of exec, device, kernel )
 ...
 * @tparam T_ConstraintTuple Tuple of constraint objects applied to parameter
 * combinations.
 */
template <typename T_Strategy = DefaultStrategy,
...
public:
   ...
   */
  explicit TuningBuilder(T_Strategy &&strategy, T_MetricInterface &&interface,
   ...
   * (special shorthand identifier for frameSpec tunables exist under the
   * tune::frame namespace -- otherwise you need to use custom identfier)
   * @tparam T_Predicate A callable predicate that returns `true` for valid
  ...
  auto withConstraint(T_Predicate &&pred) {
    ...
    auto constraint =
        tune::internal::constraint::Constraint<CleanPredicate, TuneableIDs...>{
    ...
    auto ret = TuningBuilder<T_Strategy, T_MetricInterface,
                             decltype(newConstraintTuple)>{
        std::move(m_strategy), std::move(m_metricInterface),
   ...
   *
   * The output file stores parameter configurations and their measured metrics
   * in a .json file. If this is not called or set to an empty string (`""`), no
  ...
  auto withMetricInterface(T_MetricInterfaceNew &&metricInterfaceNew) {
    using CleanMetricInterface =
        std::remove_cvref_t<T_MetricInterfaceNew>; // dont bind lvalue
    auto ret =
        TuningBuilder<T_Strategy, CleanMetricInterface, T_ConstraintTuple>{
            std::move(m_strategy),
...
private:
   ...
   *
   * Used during type transitions (e.g., when adding constraints or switching
   * strategies).
   */
  ...
  auto constraintHelper(T_Predicate pred) const {
    return tune::internal::constraint::Constraint<T_Predicate, TuneableIDs...>{
        pred};
  }
