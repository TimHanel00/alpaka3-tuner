...
template <std::integral T, uint32_t NumTunables> struct Config;
template <std::floating_point T, uint32_t NumTunables> struct NormalizedConfig;
} // namespace aTune::config
...
concept HasTraitSerializer = requires(T const &v) {
  { ::aTune::trait::Serialize<T>{}(v) } -> std::convertible_to<std::string>;
};
 ...
 *  3. Specialize a serializer under `serialize::trait` or `aTune::trait`
