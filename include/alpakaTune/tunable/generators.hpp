 ...
 */
namespace aTune::generate {
/**
 * @brief Generate a sequence using a function f
 */
namespace internal {

template <concepts::ArithmeticComparableOrVec T, typename F>
std::vector<T> generateSequence(T start, T end, T step, F &&f) {
  std::vector<T> values;
  for (T v = start; aTune::internal::utils::allTrue(v <= end); f(v, step)) {
    values.push_back(v);
  }
  return values;
}
 ...
 *
 * Generates a sequence of arithmetic values from start to end with a fixed
 * step. invoke via ()
 *
 * @tparam T Type of the sequence values. Must satisfy ArithmeticComparable
 * (+,*,=,<) concept or be a alpaka::Vec.
 */
template <concepts::ArithmeticComparableOrVec T> struct LinSpace {
   ...
   *
   * @return std::vector<T> Generated sequence -> can be used to construct a
   * tuneable object
   */
  std::vector<T> operator()() const {
    return internal::generateSequence(
        start, end, step, [](auto &val, auto &step) { val += step; });
  }
};
 ...
 *
 * Generates a sequence of arithmetic values from start to end multiplied by
 * base each step.
 *
 * @tparam T Type of the sequence values. Must satisfy ArithmeticComparable
 * (+,*,=,<) concept or be a alpaka::Vec.
 */
template <concepts::ArithmeticComparableOrVec T> struct LogSpace {
  T start; /**< Start value of the sequence */
  T end;   /**< End value of the sequence */
  ...
  std::vector<T> operator()() const {
    return internal::generateSequence(
        start, end, base, [](auto &val, auto &step) { val *= step; });
  }
};
 ...
 *
 * Constructs a generator that produces a sequence of values from `start` to
 * `end` with a fixed `step` size at runtime. Can be used to construct a
 * tuneable parameter: important this container uses std::vector, it should not
 * be used on the device side Example:
 * @code
 ...
 */
template <concepts::ArithmeticComparableOrVec T>
 ...
 *
 * Constructs a generator that produces a sequence of values from `start` to
 * `end` with a `base` multiplier at runtime. Can be used to construct a
 * tuneable parameter: important this container uses std::vector, it should not
 * be used on the device side When using this overload Note that the operation
 * cur * base is applied to all elements of a multidimensional vector at once
 * and its ensured that Vec{values...} <= range.m_end{valuesX...} for ALL values
 * ∈ Vec{values...} meaning if start{2,3} end{4,5} and stride {3,2} then {2,5}
 * is NOT generated.
 *
 * @tparam T Arithmetic type of the sequence values.
 ...
 */
template <concepts::ArithmeticComparableOrVec T>
constexpr auto linSpace(alpaka::IdxRange<T> &&range) {
 ...
 */
template <concepts::ArithmeticComparableOrVec T>
constexpr auto logSpace(alpaka::IdxRange<T> &&range) {
 ...
 * @code Example:
 * using namespace aTune::generate;
...
template <auto Start, decltype(Start) End,
          ...
          concepts::ArithmeticComparableOrVec T = decltype(Start)>
struct c_LinSpace {
  /**
  ...
  static constexpr auto generateImpl() {
    if constexpr (Cur > End)
      return std::tuple<CVecs...>{};
    else
      return generateImpl<Cur + Step, CVecs...,
                          std::integral_constant<T, Cur>>();
  }

  /// template values that can be parsed directly to a tuneable
  using values = decltype(generateImpl<Start>());
};
...
struct c_LogSpace {
  ...
  static constexpr auto generateImpl() {
    if constexpr (Cur > End)
      ...
      return generateImpl<Cur * Base, CVecs...,
                          std::integral_constant<T, Cur>>();
  }
