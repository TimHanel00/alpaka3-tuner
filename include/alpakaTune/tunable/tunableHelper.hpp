...
enum class TunableKind { TunableMD, Tunable, CTunable, Dummy };

template <std::size_t N, TunableKind kind> std::string getDefaultName() {
...
struct NoTune {
  [[nodiscard]] static NoTune copy() { return NoTune{}; }

  [[nodiscard]] static std::string toHash() { return ""; }
};

template <typename T> constexpr bool is_NoTune_v = std::is_same_v<T, NoTune>;

inline constexpr NoTune noTune{};

/*
 * basically holds and stores an ID -> used to "enable" frameTuneables without
 * specific typing, while
 */
template <auto ID> struct ShallowTunableDummy {
  ...
  auto getNumValues() const { return alpaka::Vec<uint32_t, 1u>{1u}; }
};
} // namespace aTune::internal
