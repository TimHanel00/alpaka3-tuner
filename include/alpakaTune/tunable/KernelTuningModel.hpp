 ...
 * (e.g.,Tuneable,TuneableMD,CTuneable), represented by aTune::TuneableKind.
 *
 * @details
...
struct KernelTuningModel {
  ...
  constexpr auto createNormalizedFromConfig(
    ...
    return aTune::config::NormalizedConfig<double, NumTuneables>{
        normalized_values};
  }
  ...
  auto getNumValues() const {
    ...
    meta::forEach(m_allTunables, [&]<typename T0>(T0 const &elem) {
  ...
  auto getInitConfig() {
    ...
    auto idx = 0;
    meta::forEach(m_allTunables, [&](auto const &elem) {
