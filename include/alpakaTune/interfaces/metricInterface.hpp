...
/**
* @namespace aTune::metricInterface
* @brief Defines user-extensible interfaces for performance metrics used during
tuning.
*
* The metric Interface provides a unified abstraction for measuring
* kernel performance or other user-defined evaluation criteria in the tuning
process.
*
* ### Concept
* A metric interface defines a **target function** for optimization: it measures
* how well a particular kernel configuration performs. Each metric type must
provide:
*
* - `void start()`:
*   Called before the kernel or computation begins (more specifically before
alpaka::enqueue() is called!). Used to start measurement.
*
* - `double end()`:
 ...
 */
struct Timing {
  static constexpr internal::returnComparison returnComparison{
 ...
 */
template <typename RegisterAPI_CALL> struct Occupancy {
  static constexpr internal::returnComparison returnComparison{
      internal::returnComparison::HigherIsBetter};
  ...
  explicit Occupancy(RegisterAPI_CALL &handle) : apiCaller(handle) {
    static_assert(std::is_same_v<void, void>,
                  " Occupancy is only a example of how a metric Interface "
                  "could look like not a functional "
                  "implementation "
    ...
    workerThread = std::thread([this]() {
      while (!stopFlag.load()) {
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(2)); // alpakaTune as needed
      }
    });
  }
  ...
  auto end() -> double_t {
    ...
    return std::accumulate(occupancy.begin(), occupancy.end(), 0.0) /
           occupancy.size();
  }
};
} // namespace metricInterface
...
static auto const &compareGetBest(auto const &a, auto const &b)
  requires(T_Metric::returnComparison ==
           internal::returnComparison::HigherIsBetter)
{
  if (a > b)
    return a;
