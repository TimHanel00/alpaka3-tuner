...
/// \namespace aTune::strategy
/// \brief Collection of tuning strategies.
/// A strategy chooses the next configuration, which gets enqueued for
/// evaluation.
///
/// Additional custom strategies can be defined. They must implement:
/// \code
/// auto operator()(StrategyContext<T_KernelModel, T_Metric> const& ctx)
/// noexcept;
/// \endcode
/// and return either a discrete Config (indices) or a NormalizedConfig (values
/// in [0,1]). To stop the search early, set \c ctx.env.strategyFinished = true.
namespace aTune::strategy {
namespace helper {
template <std::integral T, auto N>
void vectorFromConfig(std::vector<T> &vec, config::Config<T, N> const &config) {
  if (vec.size() != N)
    vec.resize(N);
...
template <std::floating_point T, auto N>
void vectorFromNormalizedConfig(std::vector<T> &vec,
                                config::NormalizedConfig<T, N> const &config) {
  if (vec.size() != N)
    vec.resize(N);
...
template <std::floating_point T, auto N>
inline void vectorToNormalizedConfig(config::NormalizedConfig<T, N> &config,
                                     std::vector<T> &vec) {
  if (vec.size() != N)
    vec.resize(N);
  std::copy(vec.begin(), vec.end(), config.m_values.begin());
...
// Returns a flag indicating a full wrap (all digits overflow -> back to ALL
// zeroes).
inline bool incrementMixedRadix(std::vector<uint32_t> &current,
                                std::vector<uint32_t> const &numValues) {
  if (current.size() != numValues.size()) {
    throw std::runtime_error(
        "mixed-radix increment: size mismatch (currentVals=" +
...
///
/// Use this to quickly explore the normalized space or to seed other
/// strategies.
/// \return Next \c NormalizedConfig with each dimension ~ U(0,1).
struct RandomSample {
  std::uniform_real_distribution<double> m_dist{0.0, 1.0};

  template <concepts::KernelTuningModel T_KernelModel,
            concepts::MetricInterface T_Metric>
  auto
  operator()(StrategyContext<T_KernelModel, T_Metric> const &ctx) noexcept {
    auto config =
        typename StrategyContext<T_KernelModel, T_Metric>::NormalizedConfig{};
    for (auto &val : config)
...
/// -> this strategy guarantees to find not yet evaluated config (no
/// duplicates).
///
/// Randomly samples a normalized point, maps to a discrete
/// \c Config, and advances a mixed-radix counter until an unseen config is
/// found.
...
struct RandomSearch {
  ...
  aTune::strategy::RandomSample sampler{};
...
///
/// Starts from the initial parameter configuration and walks the entire
/// mixed-radix grid. Sets \c ctx.env.strategyFinished when a full cycle is
/// completed.
...
struct ExhaustiveSearch {
  ...
  operator()(StrategyContext<T_KernelModel, T_Metric> const &ctx) noexcept {
    ...
    if (init) {
      ...
      if (!ctx.history.getOrderedHistory().empty()) {
        helper::vectorFromConfig(
            initialVals,
            ctx.history.getOrderedHistory().front().get().m_config);
      } else {
    ...
    helper::incrementMixedRadix(currentVals,
                                numValues); // uses member numValues

    // If we've wrapped back to the initial vector after at least one step,
    // finish
...
/// \brief Coarse-to-fine grid-search in normalized space with per-dimension
/// sweeps. Implements a hill-climb-ish per dimension refinement of the serach
/// space.
///
/// Sweeps each dimension over \c [start,end] in \c strideSteps steps. After a
/// sweep, snaps that dimension to the best seen (if available) and shrinks the
/// window by
/// \c zoom. Repeats for \c totalSweepCount sweeps, then sets
/// \c ctx.env.strategyFinished.
/// @Note Gives a simple heuristic of traversing dense high dimensional spaces
/// without relying on random methods.
///
...
struct IterativeRefinement {
  ...
  void initialize(StrategyContext<T_KernelModel, T_Metric> const &ctx) {
    auto norm = ctx.desc.createNormalizedFromConfig(
        ctx.history.getOrderedHistory().front().get().m_config);
  ...
  void updateCurrentValue() {
    double stepFrac =
        static_cast<double>(currentStep) / static_cast<double>(strideSteps);
    currentVals[currentDim] =
        start[currentDim] + stepFrac * (end[currentDim] - start[currentDim]);
  }
  ...
  operator()(StrategyContext<T_KernelModel, T_Metric> const &ctx) noexcept {
    ...
    if (currentStep >= strideSteps) {
      ...
      if (ctx.env.bestConfig.has_value()) {
        ...
        auto best = ctx.desc.createNormalizedFromConfig(
            ctx.env.bestConfig.value().get().m_config);
