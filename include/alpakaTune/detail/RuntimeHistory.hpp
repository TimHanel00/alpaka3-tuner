...
public:
  ...
  [[nodiscard]] auto record(double seconds) -> bool {
    if (!std::isfinite(seconds) || seconds < 0.0)
      throw std::invalid_argument{"A runtime measurement must be finite and non-negative."};
    if (isFinished())
      return true;
  ...
  void retire(RuntimeCompletion completion) {
    if (m_samples.empty())
      throw std::logic_error{"A configuration cannot retire without a measured runtime."};
  ...
  void restoreCompleted(std::span<double const> samples) {
    if (samples.empty())
      throw std::invalid_argument{"A persisted runtime history must contain a measurement."};
    m_samples.assign(samples.begin(), samples.end());
    for (auto const sample : m_samples) {
      if (!std::isfinite(sample) || sample < 0.0)
        throw std::invalid_argument{"A persisted runtime measurement is invalid."};
    }
    rebuildAcceptedSamples();
  ...
  [[nodiscard]] auto state() const noexcept -> ConfigurationState { return m_state; }
  [[nodiscard]] auto completion() const noexcept -> RuntimeCompletion { return m_completion; }
  [[nodiscard]] auto isFinished() const noexcept -> bool {
    return m_state == ConfigurationState::retired;
  }
  [[nodiscard]] auto empty() const noexcept -> bool { return m_samples.empty(); }
  [[nodiscard]] auto samples() const noexcept -> std::span<double const> { return m_samples; }
  [[nodiscard]] auto acceptedSamples() const noexcept -> std::span<double const> {
  ...
  [[nodiscard]] auto statistics() const -> RuntimeStatistics {
    ...
    auto const &accepted = m_acceptedSamples.empty() ? m_samples : m_acceptedSamples;
    ...
    auto const lowerIndex = static_cast<std::size_t>(std::max(
        0.0, std::floor((static_cast<double>(sorted.size()) -
                         m_options.ciZScore * std::sqrt(static_cast<double>(sorted.size()))) /
                        2.0)));
    auto const upperIndex = static_cast<std::size_t>(std::min(
    ...
    auto const scale = std::max(std::abs(result.median), std::numeric_limits<double>::min());
...
private:
  ...
  void validateOptions() const {
    if (m_options.minimumMeasuredRuns == 0u || m_options.maximumMeasuredRuns == 0u ||
        m_options.minimumMeasuredRuns > m_options.maximumMeasuredRuns)
      throw std::invalid_argument{"Runtime history requires 0 < minimum runs <= maximum runs."};
    if (m_options.ciCheckInterval == 0u || !std::isfinite(m_options.ciZScore) ||
        m_options.ciZScore <= 0.0 || !std::isfinite(m_options.ciRelativeWidth) ||
        m_options.ciRelativeWidth <= 0.0 || !std::isfinite(m_options.outlierMadScale) ||
        m_options.outlierMadScale <= 0.0)
      throw std::invalid_argument{"Runtime history statistical controls must be positive and finite."};
  }
 ...
 */
inline auto mannWhitneyUCompare(RuntimeHistory const &lhs, RuntimeHistory const &rhs,
  ...
  if (minimumSamples == 0u || !std::isfinite(alpha) || alpha <= 0.0 || alpha >= 1.0)
    throw std::invalid_argument{"Mann-Whitney U requires positive sample and valid alpha controls."};
  ...
  for (std::size_t begin = 0u; begin < ranked.size();) {
    auto end = begin + 1u;
    while (end < ranked.size() && ranked.at(end).first == ranked.at(begin).first)
      ++end;
    auto const count = end - begin;
    auto const rank = (static_cast<double>(begin + 1u) + static_cast<double>(end)) / 2.0;
  ...
  auto const lhsU = lhsRankSum -
                    static_cast<double>(lhsCount * (lhsCount + 1u)) / 2.0;
  auto lowerTail = 1.0;
  auto upperTail = 1.0;
  ...
  if (!hasTies && totalCount <= 20u) {
    ...
    auto enumerate = [&](auto const &self, std::size_t nextRank, std::size_t selected,
                         std::size_t rankSum) -> void {
    ...
    auto const tieFactor = 1.0 - tieCorrection / (total * total * total - total);
