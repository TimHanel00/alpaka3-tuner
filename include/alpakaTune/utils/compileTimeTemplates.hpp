...
namespace aTune::internal::compileTimeHelpers {

namespace allCombinations {
template <typename TOuter> struct ExpandTuple;

// expand a tuple of tuples to a parameter pack inserted to
// alpaka::meta::CartesianProduct
template <template <typename...> class TList, typename... Inner>
struct ExpandTuple<TList<Inner...>> {
  using type = alpaka::meta::CartesianProduct<TList, Inner...>;
};

} // namespace allCombinations

namespace createNewKernel {
template <typename T> struct ExtractTemplateArgsFromGenericKernel;
...
template <std::size_t I, typename VecT> struct index_in {
private:
  // Convert the CVec-based Vec to an integer_sequence
  using Seq =
      decltype(::alpaka::detail::toIntegerSequence(std::declval<VecT>()));

  template <std::size_t Target, std::size_t Index, typename T, T... Elems>
  struct find_index_impl;

  template <std::size_t Target, std::size_t Index, typename T, T First,
            T... Rest>
  struct find_index_impl<Target, Index, T, First, Rest...> {
    static constexpr std::size_t value =
        First == Target ? Index
                        : find_index_impl<Target, Index + 1, T, Rest...>::value;
  };
  ...
  struct find_index_impl<Target, Index, T> {
    static constexpr std::size_t value =
        static_cast<std::size_t>(-1); // Not found
  };

  template <typename T, T... Elems> struct find_index {
    static constexpr std::size_t value =
        find_index_impl<I, 0, T, Elems...>::value;
  };
  template <typename Sequence> struct get_index;
...
template <typename Tuple, typename Indices, typename Replacements,
          std::size_t CurrentKernelIndex, std::size_t NumberOfKernelArgs,
          bool Done, typename... Result>
struct rebuild_tuple_impl;

template <typename Tuple, typename Indices, typename Replacements,
          std::size_t CurrentKernelIndex, std::size_t NumberOfKernelArgs,
          typename... Result>
struct rebuild_tuple_impl<Tuple, Indices, Replacements, CurrentKernelIndex,
                          NumberOfKernelArgs, false, Result...> {
  ...
  using type = typename std::conditional_t<
      shouldReplace,
      rebuild_tuple_impl<Tuple, Indices, Replacements, CurrentKernelIndex + 1,
                         NumberOfKernelArgs, isDone, Result..., replacement>,
      rebuild_tuple_impl<Tuple, Indices, Replacements, CurrentKernelIndex + 1,
                         NumberOfKernelArgs, isDone, Result...,
                         current_T>>::type;
};
...
template <typename Tuple, typename Indices, typename Replacements,
          ...
          typename... Result>
struct rebuild_tuple_impl<Tuple, Indices, Replacements, CurrentKernelIndex,
                          NumberOfKernelArgs, true, Result...> {
...
struct ReplaceAtIndices {
  ...
  using type = typename rebuild_tuple_impl<TupleArgs, Indices, ReplacementTuple,
                                           0, N, (0 >= N)>::type;
...
struct KernelVersions<Indices, KernelArgsTuple, std::tuple<Combinations...>> {
  using type = std::tuple<typename ReplaceAtIndices<KernelArgsTuple, Indices,
                                                    Combinations>::type...>;
};
...
struct InstantiateKernelsFromTuple<Kernel, std::tuple<ArgTuples...>> {
  using type =
      std::tuple<typename ApplyTupleToTemplate<Kernel, ArgTuples>::type...>;
};
} // namespace createNewKernel
...
struct GetTemplate<Template<Args...>> {
  template <typename... Ts> using type = Template<Ts...>;
};
template <template <typename...> class Template, typename Tuple>
struct ApplyTupleToTemplate;
...
public:
  using type =
      std::tuple<typename ApplyTupleToTemplate<Template, ArgTuples>::type...>;
};
template <typename TupleOfTuples> struct TupleSizeSequence;

template <typename... Tuples> struct TupleSizeSequence<std::tuple<Tuples...>> {
...
template <typename T> struct unwrapCTunablesToTuples;

// Specialization: for a tuple of CTunables
template <typename... CTs> struct unwrapCTunablesToTuples<std::tuple<CTs...>> {
...
template <typename KernelFn> struct RegisteredCTuneables {
  using FromTrait =
      aTune::trait::CompileTimeTuneableTrait<std::decay_t<KernelFn>>;
  ...
  static_assert(numDefs == numIndices,
                "Mismatch: number of tuneable definitions must match dimension "
                "of tuned_indices");

  // unwrapps the value Tuples of the CTunable bundle.
  using unwrappedCTuneableTuples =
      typename unwrapCTunablesToTuples<TuneDefsTuple>::type;

  using Sizes =
      compileTimeHelpers::TupleSizeSequence_t<unwrappedCTuneableTuples>;
  // generates the cartesian product (all possible variant) of fixed size value
  // combination (from the compile-time tuples).
  using AllCombinations =
      typename allCombinations::ExpandTuple<unwrappedCTuneableTuples>::type;
  // get the template types from the Kernel
  using KernelTuple =
      typename createNewKernel::ExtractTemplateArgsFromGenericKernel<
          KernelFn>::type;
  ...
  using T_KernelArguments = typename createNewKernel::KernelVersions<
      decltype(FromTrait::tuned_indices), KernelTuple, AllCombinations>::type;
  // replace the std::tuple<std::tuple<Args...>...> representation of the
  // KernelArguments with actual std::tuple<Kernel<Args...>,...> types.
  using T_KernelVariants =
      typename compileTimeHelpers::InstantiateKernelsFromTuple<
          KernelFn, T_KernelArguments>::type;
};

template <typename KernelFn> constexpr auto registeredCTuneables() {
  return RegisteredCTuneables<KernelFn>{};
}

template <typename KernelFn> auto getCTunables() {
...
void runtime_tuple_dispatch_impl(std::size_t i, Tuple &tup, Fn &&fn,
...
template <typename KernelFn, std::integral IntType, auto Dim>
inline IntType
calculateColumnMajorIndex(std::array<IntType, Dim> const &indices) {
...
/**
 * @brief Applies the function @param fn to a KernelVariant. The @param indicies
 * accesses a Kernelvariant in thh cartesian product space in columnMajor order
 * (last index is slow)**/
template <typename KernelFn, auto Dim, std::integral IntType, typename Fn>
void runtime_Kernel_dispatch(std::array<IntType, Dim> const &indicies,
                             Fn &&fn) {
  static constexpr auto variants =
      typename RegisteredCTuneables<std::decay_t<KernelFn>>::T_KernelVariants{};
  static constexpr auto numVars = std::tuple_size_v<
      typename RegisteredCTuneables<std::decay_t<KernelFn>>::T_KernelVariants>;
  uint32_t columnMajorIndex =
      compileTimeHelpers::calculateColumnMajorIndex<KernelFn>(indicies);
  compileTimeHelpers::runtime_tuple_dispatch_impl(
      columnMajorIndex, variants, std::forward<Fn>(fn),
