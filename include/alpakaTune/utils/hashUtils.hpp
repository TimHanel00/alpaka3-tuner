...
/**
 * @brief Computes a byte-level hash of any trivially copyable value ( this must
 * be ensured by the caller)
 *
 * Implements the 64-bit FNV-1a hash algorithm for hashing raw object bytes.
 * Fast, constexpr-friendly, and suitable for deterministic non-cryptographic
 * hashing.
 *
 ...
 */
template <typename T> constexpr std::size_t hashBytes(T const &value) {
...
template <typename T> struct Hasher {
  ...
  std::size_t operator()(T const &val) const {
    if constexpr (concepts::Hashable<T>) {
      return std::hash<T>{}(val);
    }
    if constexpr (concepts::Serializable<T>) {
      return std::hash<std::string>{}(toStringGeneric(val));
    } else {
      static_assert(
          !std::is_same_v<T, T>,
          " Tunable types must be hashable: either specialize std::hash or "
          "provide a "
          "serialization schema for your type @see aTune::Serializable");
 ...
 */
template <typename T>
constexpr void hashCombine(std::size_t &seed, T const &value) {
  Hasher<T> hasher;
...
template <typename T>
constexpr void hashCombine(std::size_t &seed, std::vector<T> const &values) {
  for (auto const &value : values) {
    hashCombine(seed, value);
  }
}

template <typename T> std::size_t hashType(std::size_t &seed) {
  std::string name = alpaka::onHost::demangledName<T>();
