...
/**
 * @namespace aTune::internal::partitioning
...
/**
 * @brief Expand a per-dimension base stride into candidate vectors below @p
 * maxVal.
 *
 * Computes a stride per dimension as an integer multiple of @p partitionedVec,
 * then emits up to @p maxSteps vectors by repeated addition, all strictly < @p
 * maxVal. Zeros in @p partitionedVec are treated as 1. If fewer than @p
 * minSteps would fit, the stride is increased (in multiples) while staying < @p
 * maxVal.
 *
 * @return Candidates in ascending order; empty if @p minSteps==0 or @p
 * maxSteps==0.
 */
auto boundedPartitionExpansion(
  ...
  while (utils::allTrue(current < maxVal) && values.size() < maxSteps) {
...
template <alpaka::concepts::Vector T_vec,
          typename = std::enable_if_t<!std::is_integral_v<T_vec>>>
...
/**
 * @brief Component-wise multiple: largest multiples of @p baseVec not exceeding
 * @p maxVec.
 */
template <alpaka::concepts::Vector T_Vec,
          typename = std::enable_if_t<!std::is_integral_v<T_Vec>>>
...
// overload incase idxRange contains integer types instead of vec (only 1Dim
// case)
