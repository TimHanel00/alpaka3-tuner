...
namespace aTune::internal::store::detail {
#if ALPAKA_TUNE_HAS_JSON
inline nlohmann::json makeMainContextJson(KernelTuningMetadata const &md) {
  nlohmann::json meta = {{"device", md.device},
...
inline std::string computeContextID(nlohmann::json const &meta) {
  auto vec = std::vector{meta.value("device", std::string{}),
                         meta.value("executor", std::string{}),
                         meta.value("kernel", std::string{}),
                         meta.value("targetMetric", std::string{})};
  std::size_t seed = 0;
  aTune::internal::hashCombine(seed, vec);
  return std::to_string(seed);
}

// SOFT descriptor: numValues, parameters(name/id/kind/valueHash), kernelArgs,
// specifiers
template <typename... ModelArgs>
nlohmann::json
makeDescriptorJson(KernelTuningMetadata const &metaData,
                   KernelTuningModel<ModelArgs...> const &model) {
  nlohmann::json params = nlohmann::json::array();

  // Per-tunable entries including valueHash
  meta::forEach(model.m_allTunables, [&](auto const &elem) {
    params.push_back(
        {{"name", elem.m_name},
  ...
  for (std::size_t i = 0; i < model.numDims; ++i)
    desc["numValues"].push_back(
        static_cast<std::uint64_t>(model.m_numValues[i]));
...
// descriptorID := hash( specifiers[], kernelArgs,
// parameters(name,id,kind,valueHash), numValues[] )
inline std::string compute_descriptor_id(nlohmann::json const &descriptor,
                                         KernelTuningMetadata const &md) {
  std::size_t seed = 0;

  // specifiers
  if (descriptor.contains("specifiers") &&
      descriptor["specifiers"].is_array()) {
  ...
  // kernelArgs
  {
    ...
    hashCombine(seed, kernelArgs);
  }

  // parameters
  if (descriptor.contains("parameters") &&
      descriptor["parameters"].is_array()) {
    for (auto const &p : descriptor["parameters"]) {
      ...
      hashCombine(seed, kind);
      hashCombine(seed, valHash);
    }
  }
...
inline bool sameMetadata(nlohmann::json const &nodeMeta,
                         nlohmann::json const &mdWanted) {
  return nodeMeta.value("device", "") == mdWanted.value("device", "") &&
         ...
         nodeMeta.value("targetMetric", "") ==
             mdWanted.value("targetMetric", "");
}

inline bool equalStrings(std::string const &a, std::string const &b) {
  return a == b;
}

inline nlohmann::json readJsonFile(std::filesystem::path const &p) {
...
inline bool atomically_write(std::filesystem::path const &p,
...
config_to_json(aTune::config::ConfigRecord<T_Config> const &cfg, auto index) {
  nlohmann::json j;
  j["stamp"] = (cfg.state == config::ConfigState::Invalid)
...
inline bool json_has_required_context_shape(nlohmann::json const &ctx) {
  return ctx.is_object() && ctx.contains("metadata") &&
         ctx.contains("descriptor") && ctx["metadata"].is_object() &&
         ctx["descriptor"].is_object() && ctx.contains("configs") &&
         ctx["configs"].is_array() && ctx["descriptor"].contains("model") &&
         ctx["descriptor"]["model"].is_object() &&
         ctx["descriptor"]["model"].contains("numDims") &&
         ctx["descriptor"]["model"].contains("numValues");
}

inline std::string build_context_concat_string(nlohmann::json const &metadata,
                                               nlohmann::json descriptorFull) {
  ...
  oss << "device=" << metadata.value("device", "")
      << "|executor=" << metadata.value("executor", "")
      << "|kernel=" << metadata.value("kernel", "")
      << "|targetMetric=" << metadata.value("targetMetric", "");

  auto specs = metadata.value("specifiers", std::vector<std::string>{});
  if (!specs.empty()) {
    oss << "|specifiers=["
        << std::accumulate(
               std::next(specs.begin()), specs.end(), specs.front(),
  ...
  if (!params.empty()) {
    oss << "|params["
        << std::accumulate(std::next(params.begin()), params.end(),
                           formatParameter(params.front()),
                           [&](auto acc, auto const &p) {
                             return std::move(acc) + ";" + formatParameter(p);
                           })
        << "]";
  }
...
inline std::string computeContextID(nlohmann::json const &metadata,
                                    nlohmann::json const &descriptorFull) {
...
public:
  ...
  read(KernelTuningModel<ModelArgs...> const &model,
    ...
    auto root = readJsonFile(m_filename);
    if (!root.is_object()) {
      return 0;
    }

    // Identify nodes
    auto mdHard = makeMainContextJson(metadata);
    auto ctxID = computeContextID(mdHard);
    ...
    auto desc = makeDescriptorJson(metadata, model);
...
#else
    std::cerr << "[Persistent History] Could not read from history-file due to "
                 "missing JSON dependency."
              << std::endl;
    return 0;
#endif
  }
  ...
  std::size_t write(KernelTuningModel<ModelArgs...> const &model,
                    aTune::store::RuntimeHistory<T_Config> const &history,
...

  {
    ...
    for (auto const &ref : history.getOrderedHistory()) {
      ...
      if (rec.state == config::ConfigState::Initialized ||
          rec.state == config::ConfigState::Uninitialized ||
    ...
    if (!atomically_write(m_filename, root)) {
      std::cerr
          << "[PersistentHistory] ERROR: Failed to atomically write JSON file: "
          << m_filename << "\n";
      return 0;
    }
    return written;
#else
    std::cerr << "[Persistent History] Could not create history due to "
                 "json/nlohmann dependency missing or "
                 "cmake options!"
              << std::endl;
    return 0;
#endif
