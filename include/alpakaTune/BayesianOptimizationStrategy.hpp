 ...
 * bound over random normalized candidates. Context still prevents duplicate
...
private:
  ...
  [[nodiscard]] static auto kernel(ParameterConfiguration const &left,
                                   ParameterConfiguration const &right) -> double {
  ...
  [[nodiscard]] static auto solve(std::vector<double> matrix,
    ...
    for (std::size_t pivot = 0u; pivot < size; ++pivot) {
      ...
      if (std::abs(matrix[selected * size + pivot]) < noise)
        throw std::runtime_error{"Bayesian strategy received a singular kernel matrix."};
      if (selected != pivot) {
        for (std::size_t column = pivot; column < size; ++column)
          std::swap(matrix[pivot * size + column], matrix[selected * size + column]);
  ...
  [[nodiscard]] static auto buildModel(std::vector<Observation> const &observations)
      -> Model {
    ...
    for (std::size_t row = 0u; row < size; ++row) {
      ...
      for (std::size_t column = 0u; column < size; ++column) {
        matrix[row * size + column] = kernel(observations[row].configuration,
  ...
  [[nodiscard]] auto acquisition(ParameterConfiguration const &configuration,
    ...
    for (std::size_t index = 0u; index < model.size; ++index)
      covariance[index] = kernel(configuration, observations[index].configuration);
    auto mean = 0.0;
    for (std::size_t index = 0u; index < model.size; ++index)
    ...
    return mean - exploration * model.scale * std::sqrt(std::max(variance, 0.0));
  }
