 ...
 */
#pragma once
#include "alpakaTune/adjust/api.hpp"
...
#include "alpakaTune/tunable/KernelTuningModel.hpp"
...
public:
  ...
  aTune::store::RuntimeHistory<T_Config> env_activeHistory;
  ...
  aTune::core::peripherals::EnvironmentState<T_Config> env_environmentState;
  /// @brief Pending configurations to evaluate (queue).
  peripherals::ConfigQueue<aTune::config::ConfigRecord<T_Config>>
      env_config_queue;
   ...
   * otherwise.
   */
  bool violatesConstraint(aTune::config::ConfigRecord<T_Config> &configRecord) {
    ...
    bool valid = true;
    meta::forEach(this->env_constraints, [&](auto &constraint) {
...

  {
    ...
    if (env_environmentState.getBestConfig().has_value()) {
      ...
      env_config_queue.insertAdjustHead(mutableBestConfig);
      return;
    }
    ...
    aTune::config::ConfigRecord<T_Config> &firstEntry =
    ...
    if (firstEntry.state == config::ConfigState::Uninitialized) {
      ...
      if (!violatesConstraint(firstEntry)) {
        ...
        env_config_queue.insertAdjustHead(firstEntry);
      }
    }
  }
};
