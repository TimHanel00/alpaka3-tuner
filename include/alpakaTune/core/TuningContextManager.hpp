...
#include "alpakaTune/utils/transformKernelBundle.hpp"
#include <alpaka/onHost/interface.hpp>
namespace aTune::internal::core {
/**
...
private:
  ...
  applyAndExecute(T_Queue const &queue, T_Exec &&exec,
                  ...
                  aTune::config::ConfigRecord<T_Config> const &configRecord) {
    ...
    if constexpr (!trait::hasUserDefinedCTuneable<KernelFn>::value) {
      ...
      auto indicies = this->env_tuningModel.getConfigSubsetForCompileTuneables(
          configRecord.m_config);
  ...
  [[nodiscard]] auto selectNextConfig(uint32_t numAttempts = 0) {
    ...
    auto ctx = aTune::StrategyContext<decltype(this->env_tuningModel),
    ...
    aTune::config::ConfigRecord<decltype(currentConfig)> &configRecord =
  ...
  void update(aTune::config::ConfigRecord<T_Config> &stored,
