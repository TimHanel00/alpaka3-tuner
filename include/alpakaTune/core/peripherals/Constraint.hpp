...
namespace aTune::internal::constraint {
template <auto ID, typename Accessor>
constexpr auto makeAccessorOnIDMatch(Accessor const &acc) {
...
constexpr auto getAccessorForID(T_ParameterTuple const &accessorTuple) {
  ...
  return [&]<std::size_t... Is>(std::index_sequence<Is...>) {
    ...
    return std::tuple_cat(
        makeAccessorOnIDMatch<ID>(std::get<Is>(accessorTuple))...);
...
template <typename Predicate, typename Tuple> struct is_applicable {
private:
  ...
  struct impl<
      P, std::tuple<Ts...>,
      std::enable_if_t<(sizeof...(Ts) > 0) && std::is_invocable_v<P, Ts...>>>
      : std::true_type {};
 ...
 * compile-time IDs (`IDs...`) and evaluates a predicate over their current
 * values. It is used during tuning to enforce relationships between parameters
 ...
 */
template <typename T_Predicate, auto... IDs> struct Constraint {
  std::decay_t<T_Predicate> m_predicate;

  explicit Constraint(T_Predicate &&predicate)
      : m_predicate(std::forward<T_Predicate>(predicate)) {}

  template <typename Accessor>
  constexpr bool operator()(Accessor const &accessor) const {
    auto accessorTuple = constructAccessorTuple<Accessor, IDs...>(
        const_cast<Accessor &>(accessor));

    if constexpr (is_applicable_v<decltype(m_predicate),
                                  decltype(accessorTuple)>) {
      // Predicate matches tuple argument types
      return std::apply(m_predicate, accessorTuple);
    } else {
      // did not found matching IDS. This can be safely ignored if on some
      // executors that disable numThreadsTuning (when a numThreads constraint
      // was defined) @TODO think about fail safe user notification
      return true;
    }
  }
};
