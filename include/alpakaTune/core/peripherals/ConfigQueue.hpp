...
#include "alpakaTune/utils/StdMTRandomDevice.hpp"

#include <functional>
#include <optional>
#include <queue>

namespace aTune::internal::core::peripherals {

/**
 * @brief Queues parameter configurations to mitigate system noise during
 * tuning.
 *
 ...
 * noise such as thermal throttling, CPU frequency scaling, or process
 * rescheduling.
 *
 * Configurations are stored as optional references (they must outlive THE
 * queue), and the queue ensures that each entry is revisited up to a limited
 * number of consecutive times before rotating to another configuration (which
 * is selected randomly with a deterministic fallback). Retired configurations
 * are automatically recycled and removed from active scheduling.
 *
 *
 * @tparam T_Configs
 *         Type representing a configuration record (e.g. ConfigRecord or
 * similar).
 *
 *          * @note
 */
template <typename T_Configs, uint32_t maxQueueSize = MAX_QUEUE_SIZE,
          uint32_t maxConsecutiveRuns = MAX_CONSECUTIVE_RUNS>
struct ConfigQueue {
  using OptionalRef = std::optional<std::reference_wrapper<T_Configs>>;
  static constexpr uint32_t m_maxQueueSize = maxQueueSize;
  static constexpr uint32_t m_maxConsecutiveRuns = maxConsecutiveRuns;
  std::array<OptionalRef, maxQueueSize> configs;
  std::array<uint32_t, maxQueueSize> consecutiveRuns = {}; // should 0
                                                           // initialize
  ...
  static_assert(maxQueueSize >= 1u);
  static_assert(maxConsecutiveRuns > 0u);
  static_assert(maxConsecutiveRuns >
                static_cast<uint32_t>(
                    aTune::config::ConfigRecord<T_Configs>::warmUpThreshold));
  ...
  [[nodiscard]] bool empty() const noexcept { return (validCount == 0); }

  [[nodiscard]] bool full() const noexcept {
    return validCount >= maxQueueSize;
  }

  [[nodiscard]] uint32_t size() const noexcept { return validCount; }

  /**
   * @brief Inserts a configuration record into the queue.
   *
   * Each queue entry stores only the reference to a configuration object and
   * tracks its number of consecutive runs.
   *
   ...
   * prevents unnecessary allocations. If no free slots remain, we throw an
   * exception, not inserting configEntries into a full queue has to be ensured
   ...
   */
  void insert(T_Configs &config) {
    // if the strategy returns an already retired config we reset its state or
    // otherwise it will never be used by the Queue, if multiple entries of the
    // same config are in the queue the retirement of one will mean the implicit
    ...
    // if the queue is already full -- we simply return, failsafe that might
    // prevent certain configs to ever be executed, which is as a non-critical
    // error -- not overflowing the queue has to be ensured by the caller
  }

  /**
   * @brief Inserts a configuration record into the queue. And adjusts the HEAD
   * pointer to select this particular config**/
  void insertAdjustHead(T_Configs &config) {
    if (!freeSlots.empty()) {
      buildUpCount = std::min<uint32_t>(++buildUpCount, configs.size());
      uint32_t idx = freeSlots.front();
      insert(config);
...
private:
  /** @brief Called upon switching to a different config -> transition to warmUp
     state (warm up cache and pipelines with the new configuration)
  */
  ...
  /// @brief Drops an entry at idx that is retired and returns its slot to the
  /// free list.
  ...
  /** @brief Attempts to return the last used configuration if still valid and
   * under the reuse limit specified by(@c maxConsecutiveRuns).*/
  ...
  /// @brief Searches randomly up to configs.size() attempts for a valid
  /// non-retired configuration.
  std::optional<std::reference_wrapper<T_Configs>> pickRandomConfigFromQueue() {
    ...
    std::uniform_int_distribution<uint32_t> dist(
        0u, static_cast<uint32_t>(buildUpCount - 1));
  ...
  /// @brief Linearly scans all entries to find the next available non-retired
  /// configuration.
