...
namespace aTune {
/**
 * @brief Strategy context for implementing search algorithms -> A strategy
 * should aim to provide a parameter configuration.
 *
 *
 * The `StrategyContext` structure is the **main handle** that allows users to
 * implement a custom tuning strategy.  It exposes a lightweight, non-owning
 * view of the current tuning state, and performance history within a single
 * tuning context.
 *
 * The context allows strategies to:
 *   - Inspect concrete values of a parameter configuration through a @ref
 * ConfigDescriptor -> this handle also allows creating a empty zero initialized
 * config)
 *   - Check wether a Config was already evaluated with @ref history and access
 * a ConfigRecord of a paramater Configuration which tracks its measurements
 *   - Access the current tuning state of any tuning context via @ref
 * EnvironmentState, which tracks progress and break criteria.
 *   - Apply the metric policy (e.g. “lower is better” or “higher is better”) to
 * draw comparisons between ConfigRecords.
 *
 * All references are **bound** to objects owned by the tuning session; no
 * copies are made.
 *
 * @tparam T_KernelModel
 *         The kernel tuning model type that tracks all the tunable parameterd
 * and acts as the backbone of the @ref ConfigDescriptor.
 *
 * @tparam T_MetricInterface
 *         The type of the selected metric Interface. Enables comparisons
 * between ConfigRecords
 *
 *
 * @note
 ...
 *
 * @see aTune::ConfigDescriptor
 * @see aTune::core::peripherals::EnvironmentState
 * @see aTune::store::ActiveHistory
 */
template <class T_KernelModel, class T_MetricInterface> struct StrategyContext {
  ...
  /// Reference to the configuration descriptor (parameter metadata and
  /// helpers).
  ...
  /// Reference to the mutable environment state (progress, break conditions,
  /// best config).
  Environment const &env;

  void markStrategyFinished() { desc.setStrategyFinished(); }
  ...
  Record &compareGetBest(Record &a, Record &b) const noexcept {
    if constexpr (Metric::returnComparison ==
                  internal::returnComparison::LowerIsBetter)
