...
struct ExampleKernelA {
  ...
  operator()(TAcc const &acc, alpaka::concepts::MdSpan auto outputVec,
    ...
    uint32_t globalTuningIdx =
        numFramesIdx * (maxFrameExtent.product() * maxUserVal) +
        frameExtentIdx * maxUserVal + val;
...
template <class CTune> struct ExampleKernelWithCTune {
  ...
  ALPAKA_FN_ACC void operator()(TAcc const &acc,
    ...
    uint32_t globalTuningIdx =
        numFramesIdx * (maxFrameExtent.product() * maxUserVal) +
        frameExtentIdx * maxUserVal + cTuneValue;
    outputVec[globalTuningIdx] = true;
  }
};
...
struct CompileTimeTuneableTrait<ExampleKernelWithCTune<CTune>> {
  ...
  static constexpr auto tuned_indices =
      CVec<std::size_t, static_cast<std::size_t>(0)>{};
...
TEMPLATE_LIST_TEST_CASE(
  ...
  // User-provided tunable (single-dim int space), passed to the kernel via
  // KernelBundle
  Tunable<7001u, int> userTune({0, 1, 2, 3}, 2,
  ...
  auto kernelBundle = KernelBundle{
      ExampleKernelA{}, uCurrBufAcc.getMdSpan(), V2u{2, 2}, V2u{2, 2}, 4,
      userTune};
  ...
  onHost::wait(queue);
  REQUIRE(validateBuffer(uBufHost)); // validate wether we actually supplied all
                                     // of the tuning space on the
                                     // device
}
...
TEMPLATE_LIST_TEST_CASE("enqueue with CTunable", "[CTune][enqueue][full run]",
  ...
  auto kernelBundle = KernelBundle{
      ...
      uCurrBufAcc.getMdSpan(), V2u{2, 2}, V2u{2, 2}, 4};
  for (auto i = 0; i < 64 * (10 + 5);
  ...
  onHost::wait(queue);
  REQUIRE(validateBuffer(uBufHost)); // validate wether we actually supplied all
                                     // of the tuning space on the device
}
