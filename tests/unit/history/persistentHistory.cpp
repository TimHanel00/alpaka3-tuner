...
TEST_CASE("PersistentHistory round-trip write->read restores configs, samples "
  ...
  // Environment counters: checked == 3, valid stamps assigned for non-invalid
  // (2)
...
#else
TEST_CASE("PersistentHistory is skipped without JSON support",
          "[PersistentHistory][nojson]") {
