...
TEST_CASE("[TunerActiveHistory] starts empty", "") {
  ...
  REQUIRE_FALSE(hist.getRecord(config::Config<std::uint32_t, 3>{{{1, 2, 3}}})
                    .has_value());

  REQUIRE(hist.getAll().empty());
}

TEST_CASE("[TunerActiveHistory] getOrCreate inserts new entry (copy) and "
...
TEST_CASE(
    "[TunerActiveHistory] getOrCreate deduplicates by key (move overload)",
    "") {
  store::RuntimeHistory<config::Config<std::uint32_t, 3>> hist;
...
TEST_CASE("[TunerActiveHistory] getRecord present/absent behavior", "") {
  ...
  REQUIRE_FALSE(hist.getRecord(config::Config<std::uint32_t, 3>{{{9, 9, 9}}})
                    .has_value());
...
TEST_CASE("[TunerActiveHistory] getAll const and non-const access reflect same "
          "storage",
          "") {
  store::RuntimeHistory<config::Config<std::uint32_t, 3>> hist;
...
TEST_CASE("[TunerActiveHistory] References in orderedHistory remain valid "
          "across rehashes",
          "") {
  store::RuntimeHistory<config::Config<std::uint32_t, 3>> hist;
  ...
  auto it =
      hist.getAll().find(config::Config<std::uint32_t, 3>{{{10, 20, 30}}});
  REQUIRE(it != hist.getAll().end());
  REQUIRE(&it->second == &e1);
  REQUIRE(p1_from_order == &e1);
}

TEST_CASE("[TunerActiveHistory] Entries expose working MetricContainer state",
          "[ActiveHistory]") {
  store::RuntimeHistory<config::Config<std::uint32_t, 3>> hist;
  ...
  // Median can be queried (should not throw); tags live in global scope in your
  // code
