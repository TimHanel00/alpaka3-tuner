...
TEST_CASE(
    ...
    "") {
  auto spec =
      onHost::FrameSpec{Vec<uint32_t, 2u>{1, 1}, Vec<uint32_t, 2u>{2, 2}};
...
TEST_CASE("[FrameSpecTuningModel] - builder enables tuneable and get*Tune "
...
TEST_CASE("[FrameSpecTuningModel] - get*Tune returns dummy when "
          "default-enabled with no explicit tuneable",
          "") {
  auto spec =
      onHost::FrameSpec{Vec<uint32_t, 2u>{1, 1}, Vec<uint32_t, 2u>{2, 2}};
  ...
  // minimal runtime checks — we just assert these are distinct types
  // (compile-time check in templates)
...
TEST_CASE("[FrameSpecTuningModel] - partial enabling only allows get for "
          "enabled tuneables",
          "") {
  auto spec =
      onHost::FrameSpec{Vec<uint32_t, 2u>{2, 2}, Vec<uint32_t, 2u>{4, 4}};
  ...
  // Only getNumThreadsTune should compile; others should trigger static_assert
  // if used.
  [[maybe_unused]] auto nt = partial.getNumThreadsTune();
  STATIC_REQUIRE(!std::is_same_v<decltype(nt), bool>);
}

TEST_CASE("[FrameSpecTuningModel] - rvalue-qualified builder chaining and "
          "getter correctness",
          "") {
  using V2u = Vec<uint32_t, 2u>;
  auto spec = onHost::FrameSpec{V2u{1, 1}, V2u{8, 8}};
...
TEST_CASE("[FrameSpecTuningModel] - end-to-end: custom tuneables are preserved "
          "through chaining",
          "") {
  using V2u = Vec<uint32_t, 2u>;
...
TEST_CASE("[FrameSpecTuningModel] - builder chaining: all tuneables retrieved "
          "correctly",
          "") {
  using V2u = Vec<uint32_t, 2u>;
  auto spec = onHost::FrameSpec{V2u{1, 1}, V2u{8, 8}};
...
TEST_CASE("[FrameSpecTuningModel] - test argument forwarding constructor.",
  ...
  auto model = FrameSpecTuningModel{spec}
                   .withNumFramesTune(stType{V2u{1, 1}, V2u{2, 2}},
                                      std::nullopt, "Tune1")
                   .withFrameExtentTune(stType{V2u{1, 1}, V2u{2, 2}},
                                        std::nullopt, "Tune2")
                   .withNumBlocksTune(stType{V2u{1, 1}, V2u{2, 2}},
                                      std::nullopt, " Tune3")
                   .withNumThreadsTune(stType{V2u{1, 5}, V2u{1, 9}},
                                       std::nullopt, "Tune4");
  ...
  auto model2 = FrameSpecTuningModel{spec}
                    .withNumFramesTune(stType2{V2u{1, 1}, V2u{2, 2}},
                                       std::nullopt, "Tune1")
                    .withFrameExtentTune(stType2{V2u{1, 1}, V2u{2, 2}},
                                         std::nullopt, "Tune2")
                    .withNumBlocksTune(stType2{V2u{1, 1}, V2u{2, 2}},
                                       std::nullopt, " Tune3")
                    .withNumThreadsTune(stType2{V2u{1, 5}, V2u{2, 3}},
                                        std::nullopt, "Tune4");
...
TEST_CASE("[FrameSpecTuningModel] - withALL shallowTunables, order of calls is "
          "interchangable",
          "") {
  using V2u = Vec<uint32_t, 2u>;
  auto spec = onHost::FrameSpec{V2u{1, 1}, V2u{8, 8}};
