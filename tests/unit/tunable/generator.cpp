...
TEST_CASE("Compile-time linear space generates correct tuple", "[c_LinSpace]") {
  ...
  using ExpectedTuple = std::tuple<
      std::integral_constant<type, 2>, std::integral_constant<type, 3>,
      std::integral_constant<type, 4>, std::integral_constant<type, 5>>;
  static_assert(std::is_same_v<seq_T, ExpectedTuple>);
}

TEST_CASE("Compile-time logarithmic space generates correct tuple",
          "[c_LogSpace]") {
  using type = decltype(1);
  constexpr auto seq = generate::c_LogSpace<1, 9, 3>::values{};
  using ExpectedTuple = std::tuple<std::integral_constant<type, 1>,
                                   std::integral_constant<type, 3>,
                                   std::integral_constant<type, 9>> const;
...
TEST_CASE("Tuneable compatible", "[c_LogSpace]") {
  using type = decltype(1);
  using TuneTuple = CTunable<static_cast<std::size_t>(0),
                             generate::c_LogSpace<1, 9, 3>::values>::Values;
  using ExpectedTuple = std::tuple<std::integral_constant<type, 1>,
                                   std::integral_constant<type, 3>,
