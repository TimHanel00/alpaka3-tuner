...
template <typename CVec_> struct TestKernel {
  ...
  auto getValue() const { return CVec_{}; }
};
...
template <typename T, typename CVec_, typename CVec2_> struct TestKernelMD {
  ...
  auto getValue_1() const { return CVec_{}; }

  auto getValue_2() const { return CVec2_{}; }
};

struct foo {
  template <typename T_Other> bool operator==(T_Other const &other) {
...
template <typename T, typename A, typename B> struct TestKernelArbitrary {
  ...
  auto getValue_1() const { return T{}; }

  auto getValue_2() const { return A{}; }

  auto getValue_3() const { return B{}; }
};
...
template <typename T> struct CompileTimeTuneableTrait<TestKernel<T>> {
  ...
  static auto tuneAbleDefinitions() {
    auto tune1 = tune::CTunable<static_cast<std::size_t>(0), alpaka::CVec<t, 1>,
                                alpaka::CVec<t, 2>, alpaka::CVec<t, 8>,
                                alpaka::CVec<t, 10>>{};
...
struct CompileTimeTuneableTrait<TestKernelThreeDim<T, CVec_type, T2>> {
  ...
  static auto tuneAbleDefinitions() {
    auto tune1 = tune::CTunable<static_cast<std::size_t>(6), alpaka::CVec<t, 1>,
                                alpaka::CVec<t, 2>, alpaka::CVec<t, 8>,
                                alpaka::CVec<t, 10>>{};
    return std::tuple{tune1};
  }
...
struct CompileTimeTuneableTrait<TestKernelMD<T, CVec_type, CVec_type2>> {
  ...
  static auto tuneAbleDefinitions() {
    auto tune1 = tune::CTunable<static_cast<std::size_t>(6), alpaka::CVec<t, 1>,
                                ...
                                alpaka::CVec<t, 10>>{};
    auto tune2 =
        tune::CTunable<static_cast<std::size_t>(12), alpaka::CVec<uint32_t, 15>,
                       alpaka::CVec<uint32_t, 20>,
...
struct CompileTimeTuneableTrait<TestKernelArbitrary<T, A, B>> {
  ...
  static auto tuneAbleDefinitions() {
    ...
    auto tune3 = tune::CTunable<140u /***ID***/, alpaka::CVec<uint32_t, 1u, 2u>,
...
TEST_CASE("parseCompileTimeTuneables", "[KernelVariantGeneration]") {
  ...
  [[maybe_unused]] static auto variants =
      typename tune::internal::compileTimeHelpers::RegisteredCTuneables<
  ...
  tune::meta::for_each_enumerate(variants, [&]<typename T0>(T0 const &val,
                                                            auto idx) {
    static_assert(
        std::is_convertible_v<typename T0::Param1, alpaka::CVec<uint32_t, 18>>);
    tune::meta::visitIndex(
