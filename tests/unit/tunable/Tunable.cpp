...
TEST_CASE("CTuneable basic compile-time tuple", "[CTuneable]") {
  using t = uint32_t;
  using Tune =
      CTunable<10, alpaka::CVec<t, 0>, alpaka::CVec<t, 1>, alpaka::CVec<t, 2>>;
...
TEST_CASE("CTuneable runtime interface", "[CTuneable]") {
  ...
  CTunable<id, alpaka::CVec<t, 10>, alpaka::CVec<t, 11>, alpaka::CVec<t, 24>>
      tuneDefault;
  ...
  CTunable<12, alpaka::CVec<t, 10>, alpaka::CVec<t, 11>, alpaka::CVec<t, 24>>
      tuneNamed("MyTune");
...
TEST_CASE("CTuneable supports user-defined types", "[CTuneable]") {
  struct Foo {
    ...
    constexpr bool operator==(Foo const &) const { return true; }
  };
...
TEST_CASE("TunableMD - construction from initializer list", "[TunableMD]") {
  TunableMD tmd{{{1u, 10u}, {2u, 20u}, {3u, 15u}},
                std::optional<Vec2u>{{2u, 20u}},
                "InitListTuneable"};
...
TEST_CASE("TunableMD - findStartingIndex valid value", "[TunableMD]") {
  Vec2u start{2u, 20u};
  TunableMD<alpaka::uniqueId(), Vec2u> tmd{
      {{1u, 10u}, {2u, 20u}, {3u, 30u}}, start, ""};
...
TEST_CASE("TunableMD - findStartingIndex invalid value", "[TunableMD]") {
  ...
  TunableMD<alpaka::uniqueId(), Vec2u> tmd{{{1u, 10u}, {2u, 20u}, {3u, 30u}},
                                           start};
...
TEST_CASE("TunableMD - 3D consistency check", "[TunableMD]") {
  std::vector<Vec3u> space = {
      {1u, 10u, 100u}, {2u, 20u, 200u}, {3u, 30u, 300u}};
...
TEST_CASE("Tunable constructed from IdxRange", "[Tunable]") {
  ...
  auto tune =
      Tunable<alpaka::uniqueId(), Vec1>(range, Vec1{3u}, "RangeTuneable");
...
TEST_CASE("valuesToHash across tunables: stability and separation",
          "[valuesToHash]") {
  SECTION("CTunable (compile-time) — same types/values => same hash; different "
          "=> different") {
    using T = uint32_t;

    using C1 = CTunable<alpaka::uniqueId(), alpaka::CVec<T, 1>,
                        alpaka::CVec<T, 2>, alpaka::CVec<T, 3>>;
    using C2 = CTunable<alpaka::uniqueId(), alpaka::CVec<T, 1>,
                        alpaka::CVec<T, 2>, alpaka::CVec<T, 3>>; // different
                                                                 // tag, same
                                                                 // payload
    using C3 = CTunable<alpaka::uniqueId(), alpaka::CVec<T, 1>,
                        alpaka::CVec<T, 2>, alpaka::CVec<T, 4>>; // one value
                                                                 // different

    // currently this is not constexpr (mainly because of the type name which is
    // part of the hash)
    std::size_t h1 = C1{}.valuesToHash();
    std::size_t h2 = C2{}.valuesToHash();
    std::size_t h3 = C3{}.valuesToHash();

    REQUIRE(h1 == h2); // same types and compile-time values -> same hash (tag
                       // is not included)
    REQUIRE(h1 != h3); // changed one alpaka::CVec value/type -> different hash
  }

  SECTION("Tunable (runtime 1D) — same values => same hash; different => "
          "different") {
    Tunable t1({1, 2, 3});
    Tunable t2({1, 2, 3});
  ...
  SECTION("TunableMD (runtime multi-dim) — same spaces => same hash; different "
          "=> different") {
