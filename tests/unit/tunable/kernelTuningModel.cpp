...
TEST_CASE("tune::internal::KernelTuningModel - getValuesFromConfig builds "
  ...
  // ------------------------------------------------------------------------
  constexpr std::size_t dims = decltype(m)::numDims;
  REQUIRE(dims == 1       /*numBlocks Vec<1>*/
                      + 2 /*ThreadBlock Vec<2>*/
                      + 1 /*user*/
                      + 1 /*CTunable*/);
  ...
  REQUIRE(cfg[0] == 2u);
  REQUIRE(cfg[3] == 1u);
  /*    auto u = std::tuple{Tunable<6001, uint32_t>{{3u, 6u, 9u}, 6u,
  "factor"}};

  // frame tuneables (1D + 2D)
  auto f = std::tuple{
  Tunable<frame::numBlocks>{{{1u}, {2u}, {4u}}, std::nullopt, "Blocks"},
  TunableMD<frame::ThreadBlock>{{{8u, 4u}, {16u, 8u}}, std::nullopt,
  "Threads"}};
...
TEST_CASE("tune::internal::KernelTuningModel - applyToFrameSpec writes fields "
  ...
  // Pick: numBlocks=idx1->{4}, NumFrames={1,0}->{2,1},
  // ThreadBlock={1,1}->{16,8}, FrameExtent={0,1}->{128,128}
  config::Config<uint32_t, decltype(m)::numDims> cfg{
...
TEST_CASE("tune::internal::KernelTuningModel - "
  ...
  STATIC_REQUIRE(std::is_same_v<decltype(ctSubset),
                                std::array<uint32_t, std::tuple_size_v<CT>>>);
