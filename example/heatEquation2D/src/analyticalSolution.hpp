...
//! u_t(x, y, t) = u_xx(x, t) + u_yy(y, t), x in [0, 1], y in [0, 1], t in [0,
//! T]
//!
//! \param x value of x
//! \param x value of y
//! \param t value of t
ALPAKA_FN_HOST_ACC auto exactSolution(double const x, double const y,
                                      double const t) -> double {
  constexpr double pi = alpaka::math::constants::pi;
  return alpaka::math::exp(-pi * pi * t) *
         (alpaka::math::sin(pi * x) + alpaka::math::sin(pi * y));
}

//! Validate calculated solution in the buffer to the analytical solution at
//! t=tMax
//!
...
auto validateSolution(T_MdSpan const dataMdSpan, T_Extent const &extent,
                      double const dx, double const dy, double const tMax)
    -> std::pair<bool, double> {
  // Calculate error
  double maxError = 0.0;
  for (uint32_t j = 1; j < extent[0] - 1; ++j) {
    for (uint32_t i = 1; i < extent[1] - 1; ++i) {
      auto const error = std::abs(dataMdSpan[alpaka::Vec{
                                      j,
                                      i,
                                  }] -
...
auto initalizeBuffer(T_MdSpan dataMdSpan, double const dx, double const dy)
    -> void {
