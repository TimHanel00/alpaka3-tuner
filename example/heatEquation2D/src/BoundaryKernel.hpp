...
struct BoundaryKernel {
  ...
  ALPAKA_FN_ACC auto
  operator()(TAcc const &acc, alpaka::concepts::IMdSpan auto uBuf,
             alpaka::concepts::Vector auto numNodes, uint32_t step,
    ...
    for (auto [x] : onAcc::makeIdxMap(acc, onAcc::worker::linearThreadsInGrid,
                                      IdxRange{Vec{numNodes.x()}})) {
      auto nodeIdxBegin = Vec{Idx{0u}, x};
      uBuf[nodeIdxBegin] = exactSolution(nodeIdxBegin.x() * dx,
                                         nodeIdxBegin.y() * dy, step * dt);
      auto nodeIdxEnd = Vec{numNodes.y() - Idx{1u}, x};
      uBuf[nodeIdxEnd] =
          exactSolution(nodeIdxEnd.x() * dx, nodeIdxEnd.y() * dy, step * dt);
    }

    // move over Y surfaces, skip first and last node
    for (auto [y] :
         onAcc::makeIdxMap(acc, onAcc::worker::linearThreadsInGrid,
                           IdxRange{Vec{1u}, Vec{numNodes.y() - 1u}})) {
