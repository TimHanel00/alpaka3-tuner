...
public:
  ...
  void tuneVectorAdd(alpakaTune::StrategyKind strategy, std::size_t queueWindow,
    ...
    auto const vectorAdd = alpaka::KernelBundle{VectorAddKernel{}, m_inputA,
                                                m_inputB, m_output, extent};
    for (std::size_t step = 0u; step < NumberOfInsideSteps; ++step) {
      tuner.enqueue(m_queue, m_frameSpec, vectorAdd);
      ++m_innerEnqueueCount;
    }
  }
...
struct TuneTheTunerKernel {
  template <typename Accelerator, typename State, typename Strategy,
            typename QueueWindow, typename ConsecutiveRuns>
  ALPAKA_FN_ACC void operator()(Accelerator const &, State *state,
                                Strategy strategy, QueueWindow queueWindow,
                                ConsecutiveRuns consecutiveRuns) const {
#if defined(__CUDA_ARCH__) || defined(__HIP_DEVICE_COMPILE__)
    (void)state;
#else
    state->tuneVectorAdd(strategy, queueWindow, consecutiveRuns);
#endif
  }
};
...
auto main() -> int {
  ...
  auto const tuneTheTuner = alpaka::KernelBundle{
  ...
  for (std::size_t step = 0u; step < NumberOfOutsideSteps; ++step)
    outerTuner.enqueue(outerQueue, outerFrameSpec, tuneTheTuner);

  auto const valid = innerTuner.configurationCount() == NumberOfOutsideSteps &&
                     innerTuner.innerEnqueueCount() ==
                         NumberOfOutsideSteps * NumberOfInsideSteps &&
                     innerTuner.resultIsCorrect();

  std::cout << "Tuned " << innerTuner.configurationCount()
            << " inner-tuner configurations with "
            << innerTuner.innerEnqueueCount() << " vector-add launches.\n";
