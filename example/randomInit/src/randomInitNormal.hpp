...
struct RandomInitKernelNormal {
  ...
  ALPAKA_FN_ACC void operator()(auto const &acc, auto outArray, auto mean,
                                auto stdDev) const {
    ...
    for (auto [seed] :
      ...
      for (auto [index] : alpaka::onAcc::makeIdxMap(
               acc, workGroup, alpaka::IdxRange{outArray.getExtents()})) {
...
bool validateNormal(auto const &data, auto expectedMean, auto expectedStdDev) {
  ...
  double m4 = sum4 / n - 4 * mean * sum3 / n + 6 * mean * mean * sum2 / n -
              3 * pow(mean, 4);
  ...
  // Acceptable tolerance for mean and stddev (adjust as needed for your sample
  // size)
  double meanTol = 0.01;
...
int exampleDispatch(auto const cfg, uint32_t numElements, auto const &mean,
                    auto const &stdDev) {
  using T_Data = float;
  std::cout << "\nTesting normal distribution.\n";
  // Limit numElements with numElementsNormal since whole array is filled, not
  // only histogram is tested
  if (numElements > numElementsNormal) {
    std::cout << "Limiting number of elements for normal distribution test: "
              << numElementsNormal << "\n";
  ...
  std::vector const data(std::data(outArray_h),
                         std::data(outArray_h) + numElements);
  bool resultIsCorrect = validateNormal(data, mean, stdDev);

  if (resultIsCorrect) {
    std::cout
        << "Execution results of normal dist correct with a certain confidence!"
        << std::endl;
    return EXIT_SUCCESS;
  }

  std::cout << "Execution results incorrect: Random numbers do not follow the "
...
int exampleNormalDist(auto const cfg, uint32_t numElements) {
  ...
  for (auto const &param : params) {
    if (EXIT_SUCCESS !=
        exampleDispatch(cfg, numElements, param.first, param.second)) {
      return EXIT_FAILURE;
    }
  }
  return EXIT_SUCCESS;
