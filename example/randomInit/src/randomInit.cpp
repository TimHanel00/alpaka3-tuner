...
bool validateHistogram(auto const &histogram, int32_t const expectedValue) {
  ...
  std::cerr << "Max value: " << maxValue << ", Min value: " << minValue
            << ", Difference: " << maxValue - minValue
            << ", Allowed difference: " << allowedDifference << "\n";
  if (!isValid) {
    std::cerr << "Histogram validation failed: Difference between max and min "
                 "values is too large.\n";
  }
...
//! This kernel uses an uniform random integer generator and uses a bit
//! manipulator-scaler to generate uniform float random numbers in the range [0,
//! 1). The generated random numbers are then scaled to fit into the histogram
//! bins. The histogram is filled by atomically incrementing the bin count for
//! each generated number. The kernel uses the Philox engine to generate random
//! numbers.
struct RandomInitKernelUniform {
  ...
  ALPAKA_FN_ACC void operator()(auto const &acc, auto outBins,
                                uint32_t size) const {
    ...
    for (auto [seed] :
         alpaka::onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                   alpaka::onAcc::range::threadsInGrid))
    // Initialize the Philox engine with the global frame element ID as the seed
    {
      ...
      for ([[maybe_unused]] auto [index] :
           alpaka::onAcc::makeIdxMap(acc, workGroup, alpaka::IdxRange{size})) {
        // Generate numbers in [0,1). Philox engine already generates uniform
        // random numbers but they are integers.
        ...
        // Since the values are in [0,1), we multiply by the number of bins to
        // get the bin index The bin index is in the range [0, numberOfBins)
...
struct RandomInitKernel {
  //! The kernel operator uses the Philox engine to generate uniform 32-bit
...
// Kernel to generate random numbers and fill the histogram
// This kernel uses a vectorized approach to generate 4 integer random numbers
// at once. Results are collected in bins to generate the histogram
...
bool testRandomInitKernels(alpaka::onHost::concepts::Device auto host,
                           alpaka::onHost::concepts::Device auto device,
                           auto computeExec, uint32_t numElements) {
  ...
  tuneHistogram(
      philoxTuning,
      alpaka::KernelBundle{RandomInitKernel{}, outBins_d.getMdSpan(), size});
  ...
  std::vector<T_Data> randomInitHist(std::data(outBins_h),
                                     std::data(outBins_h) + numBins);
  ...
  auto validateResult = [&](char const *kernelName, auto &histogram) {
    ...
    if (isHistogramValid) {
      std::cout << "Histogram Validation passed: Histogram distribution is "
                   "within the expected tolerance.\n";
    } else {
      std::cerr << "Histogram Validation failed: Histogram distribution is not "
                   "within the expected tolerance.\n";
    }
  ...
  // TEST - 2 Test RandomInitKernelUniform which generates real random numbers
  // in [0,1) Fill the histogram buffer with zeros
  for (uint32_t i = 0; i < numBins; ++i) {
    outBins_h[i] = 0;
  }
  ...
  std::cout << "- Testing RandomInitKernelUniform with a grid of " << frameSpec
            << "\n";
  tuneHistogram(uniformTuning,
                alpaka::KernelBundle{RandomInitKernelUniform{},
                                     outBins_d.getMdSpan(), size});
  ...
  std::vector<T_Data> randomInitUniformHist(std::data(outBins_h),
                                            std::data(outBins_h) + numBins);
  ...
  tuneHistogram(
      vectorTuning,
      alpaka::KernelBundle{RandomInitKernelVec{}, outBins_d.getMdSpan(), size});
  ...
  // Validate Results
  std::vector randomInitVecHist(std::data(outBins_h),
                                std::data(outBins_h) + numBins);
...
int exampleUniformDist(auto const cfg, size_t numElements) {
  ...
  auto hostSelector =
      alpaka::onHost::makeDeviceSelector(api::host, deviceKind::cpu);
  ...
  bool resultIsCorrect =
      testRandomInitKernels(host, device, computeExec, numElements);

  if (resultIsCorrect) {
    std::cout << "Execution results correct with a certain confidence!"
              << std::endl;
    return EXIT_SUCCESS;
  } else {
    std::cout << "Execution results incorrect: Random numbers do not follow "
...
void help(char *argv[]) {
  ...
  std::cerr << "  -n  numElements: Number of elements to process. Default:"
  ...
  return onHost::executeForEach(
      [=](alpaka::concepts::BackendSpec auto const &backend) {
        ...
        if (!selector.isAvailable())
          return EXIT_SUCCESS;
        bool retVal = exampleUniformDist(backend, numElements) ||
                      exampleNormalDist(backend, numElementsNormal);
