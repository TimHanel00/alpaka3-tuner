...
public:
  ALPAKA_FN_ACC void operator()(auto const &acc,
                                alpaka::concepts::IMdSpan auto argb,
                                auto const numElements) const {
    constexpr uint32_t maskFF = 0xFFu;
    auto simdGrid = onAcc::SimdAlgo{onAcc::worker::threadsInGrid};

    simdGrid.concurrent(
        acc, alpaka::Vec{numElements},
        [&](auto const &, auto &&simdARGB) constexpr {
          ...
          auto simdGray =
              (simdR * scalarR + simdG * scalarG + simdB * scalarB) / scalar32;

          // Reconstruct ARGB value. Write same value to 3 bytes and alpha to
          // the forth byte
          simdARGB =
              simdGray | (simdGray << 8u) | (simdGray << 16u) | (simdA << 24u);
        },
        argb);
  }
};
...
void naiveToGray(auto &&buffer) {
  std::for_each(reinterpret_cast<Pixel *>(onHost::data(buffer)),
                reinterpret_cast<Pixel *>(onHost::data(buffer) +
                                          onHost::getExtents(buffer).x()),
                [](Pixel &pixel) {
                  auto const gray =
                      (pixel.r * 11 + pixel.g * 16 + pixel.b * 5) / 32;
                  pixel.r = pixel.g = pixel.b = gray;
                });
}

auto validateResult(auto &&bufHostScalarRGB, auto const &bufHostR,
                    auto const &bufHostG, auto const &bufHostB,
                    auto const &bufHostA, size_t extent) {
  using Data = uint32_t;
  ...
  for (size_t i(0u); i < extent; ++i) {
    Data const &computedGray(
        bufHostScalarRGB[i]); // Direct indexing for scalarRGB

    uint32_t const grayValue =
        static_cast<uint8_t>((scalarR * bufHostR[i] + // Direct indexing for R
                              scalarG * bufHostG[i] + // Direct indexing for G
                              scalarB * bufHostB[i])  // Direct indexing for B
                             / scalar32);             // Normalize by scalar32

    Data correctResult = grayValue | (grayValue << 8u) | (grayValue << 16u) |
                         (bufHostA[i] << 24u);

    if (computedGray != correctResult) {
      if (falseResults < MAX_PRINT_FALSE_RESULTS)
        std::cerr << "RGBA[" << i << "] == " << std::hex << computedGray
                  << " != " << correctResult << std::endl;
  ...
  if (falseResults == 0) {
    ...
    std::cout << "Found " << falseResults
              << " false results, printed no more than "
...
auto example(T_Cfg const &cfg, size_t numElements, size_t numberOfRuns,
             bool enableStdForEach) -> int {
  ...
  std::cout << "    Element type [byte]: " << onHost::demangledName<Data>()
            << std::endl;
  std::cout << "    Buffer size [Gbyte]: " << numElements * sizeof(Data) / 1.e9
            << std::endl;
  ...
  for (auto i(0u); i < extent; ++i) {
    ...
    bufHostB[i] = dist(eng);
    bufHostA[i] = dist(eng);
    Data const argbValue = (static_cast<Data>(bufHostA[i]) << 24u) |
                           (static_cast<Data>(bufHostR[i]) << 16u) |
                           (static_cast<Data>(bufHostG[i]) << 8u) |
  ...
  if (exec == alpaka::exec::cpuSerial && enableStdForEach) {
    ...
    std::cout << "    Time for kernel execution [s]: " << kernelRuntime
              << std::endl;
    std::cout << "    Processed [Gbyte/s]: "
              << (static_cast<double>(2ull * numElements * sizeof(Data)) /
                  kernelRuntime) /
                     1.e9
              << std::endl;

    int result = validateResult(bufHostScalarRGB, bufHostR, bufHostG, bufHostB,
                                bufHostA, extent.x());
  ...
  std::cout << "Using alpaka accelerator: " << onHost::demangledName(exec)
            << " for " << deviceSpec.getApi().getName() << std::endl;
  ...
  for (; alpakaTune::example::applicationRunsRemain(completedRuns, numberOfRuns,
    ...
    double const kernelRuntime =
        std::chrono::duration<double>(endKernelT - beginKernelT).count();
    ...
    double const copyRuntime =
        std::chrono::duration<double>(endCopyT - beginCopyT).count();
    totalCopyRuntime += copyRuntime;

    if (completedRuns == 0) {
      int result = validateResult(bufHostARGB, bufHostR, bufHostG, bufHostB,
                                  bufHostA, extent.x());
      if (result != EXIT_SUCCESS)
        return result;
    }
  ...
  if (completedRuns > 0u) {
    double const avgKernelRuntime =
        totalKernelRuntime / static_cast<double>(completedRuns);
    std::cout << "    Time for kernel execution [s]: " << avgKernelRuntime
              << std::endl;
    std::cout << "    Processed [Gbyte/s]: "
              << (static_cast<double>(2llu * numElements * sizeof(Data)) /
                  avgKernelRuntime) /
                     1.e9
              << std::endl;

    double const avgCopyRuntime =
    ...
    std::cout << "    Copied [Gbyte/s]: "
              << (static_cast<double>(numElements * sizeof(Data)) /
                  avgCopyRuntime) /
                     1.e9
              << std::endl;
  }
  ...
  while ((opt = getopt(argc, argv, "hn:er:")) != -1) {
    ...
    case 'r':
      try {
        ...
        std::cerr << "Error: number of runs '" << optarg
                  << "' out of range for size_t.\n";
        return EXIT_FAILURE;
      }
      if (numberOfRuns == 0) {
