...
/** @brief The main kernel, computing one step of the simulation for all
 * particles.
 *
 ...
 * @param chunkSize The chunk size for the calculation (1D). Will also be used
 * as shared memory size, so it has to be a CVector (known at compile time).
 * @param dt The delta t time step to compute.
 *
 * @note No boundary checks are performed in this kernel, the extents are
 * assumed to be a multiple of the chunk size.
 */
struct UpdateVelocitiesKernel {
  template <typename T_Acc, typename T_View>
  ALPAKA_FN_ACC auto operator()(T_Acc const &acc,
    ...
    for (auto blockStartIdx : onAcc::makeIdxMap(
             acc, onAcc::worker::blocksInGrid,
             IdxRange{CVec<IdxType, 0_idx>{}, extents, chunkSize})) {
      // CUDA 13.1/13.2 bug workaround: concepts can not be used in a range
      // based for loop
      ...
      auto particlePositions =
          onAcc::declareSharedMdArray<Simd<BaseType, 3u>, uniqueId()>(
              acc, chunkSize);
      ...
      // the data of the other particles for each tile. will be overridden in
      // the inner loop (3-dimensional positions + mass)
      auto otherParticlePositions =
          onAcc::declareSharedMdArray<Simd<BaseType, 3u>, uniqueId()>(
              acc, chunkSize);
      auto masses =
          onAcc::declareSharedMdArray<BaseType, uniqueId()>(acc, chunkSize);

      // == load the particles for this block into shared memory and initialize
      // accelerations ==
      for (auto particleIdx : onAcc::makeIdxMap(
               acc, onAcc::worker::threadsInBlock, IdxRange{chunkSize})) {
        // CUDA 13.1/13.2 bug workaround: concepts can not be used in a range
        // based for loop
        ...
        particlePositions[particleIdx] =
            Simd{particleData.xPositions[globalIdx],
                 particleData.yPositions[globalIdx],
                 particleData.zPositions[globalIdx]};
        accelerations[particleIdx] =
            Simd{BaseType{0.}, BaseType{0.}, BaseType{0.}};
      }
      ...
      for (auto otherBlockStartIdx = 0_idx; otherBlockStartIdx < extents.x();
        ...
        // == load the particles for this block into shared memory and
        ...
        for (auto particleIdx : onAcc::makeIdxMap(
          ...
          otherParticlePositions[particleIdx] =
              Simd{particleData.xPositions[otherGlobalIdx],
        ...
        for (auto particleIdx : onAcc::makeIdxMap(
          ...
          for (IdxType i = 0_idx; i < chunkSize; ++i) {
            auto const distanceVector =
                otherParticlePositions[i] - particlePositions[particleIdx];
      ...
      // == calculate and save the particles' new velocities back into global
      // memory ==
      for (auto particleIdx : onAcc::makeIdxMap(
               acc, onAcc::worker::threadsInBlock, IdxRange{chunkSize})) {
        // CUDA 13.1/13.2 bug workaround: concepts can not be used in a range
        ...
        particleData.xVelocities[blockStartIdx + particleIdx] +=
            acceleration.x();
        particleData.yVelocities[blockStartIdx + particleIdx] +=
            acceleration.y();
        particleData.zVelocities[blockStartIdx + particleIdx] +=
            acceleration.z();
      }
    }
  }
...
struct UpdatePositions {
  ...
  constexpr void
  operator()(concepts::SimdPtr auto xPos, concepts::SimdPtr auto yPos,
             concepts::SimdPtr auto zPos, concepts::SimdPtr auto xVel,
             concepts::SimdPtr auto yVel, concepts::SimdPtr auto zVel) const {
