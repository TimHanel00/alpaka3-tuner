...
void writePng(ParticleData<T_View> const &particles,
  ...
  for (IdxType idx = 0; idx < particles.getExtents().x(); ++idx) {
    ...
    // Ignore z coordinate
    auto const px = (p.xPos - minParticlePos) /
                    (maxParticlePos - minParticlePos) * screenWidth;
    auto const py = (p.yPos - minParticlePos) /
                    (maxParticlePos - minParticlePos) * screenHeight;
    ...
    if (zDistance <
        zClipNear) // skip particles that are too close to the camera
      continue;

    // scale dot size inversely with distance to camera
    BaseType const scaledDistance =
        zDistance / (maxParticlePos - minParticlePos);
