 ...
 */
template <concepts::IMdSpan<BaseType> T_View> void initMasses(T_View &masses) {
...
/** @brief Helper function to generate a tangential velocity to prevent the
 * whole set of particles moving in a random direction.
 */
auto randomTangentialVelocity(auto &rd, auto &distribution, BaseType const x,
                              BaseType const y, BaseType const z) {
  // Random vector
...
/** @brief Initialize the given x, y, and z-velocities with random values, given
 * the positions. The positions are used to generate tangential velocities to
 * the coordinate center.
 * @note This is a host function.
 */
template <concepts::IMdSpan<BaseType> T_View>
void initVelocities(T_View &xVelocities, T_View &yVelocities,
                    T_View &zVelocities, T_View const &xPositions,
  ...
  for (auto i = 0_idx; i < xVelocities.getExtents().x(); ++i) {
    auto const tangentialVelocity = randomTangentialVelocity(
        rd, distribution, xPositions[i], yPositions[i], zPositions[i]);
...
template <concepts::IMdSpan<Color> T_View> void initColors(T_View &colors) {
  auto rd = std::random_device{};
