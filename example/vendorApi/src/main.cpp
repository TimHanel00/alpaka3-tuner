 ...
 *
 * The function interface is a mechanism to register and dispatch a function
 * interface from alpaka to third party libraries for specific APIs and device
 * kinds.
 *
 * The alpaka function symbols provides you with a clean unified way to dispatch
 * a user chosen function interface signature to functions of different vendors.
 * There is no need to deal with preprocessor macros on the caller side within
 * user functions, which keeps the code readable. You could create quite similar
 * results with C++ functor trait specializations but with the drawback that you
 * must deal with type signature on the caller side instead of having a
 * well-defined way how the function call is dispatched to the best fitting
 * overload.
 *
 * The example shows how to register and dispatch a vendor function for the api
 * Host and device kind Cpu, which is implemented with std::transform. If
 * available, a vendor function overload for the api Cuda and device kind
 * NvidiaGpu is registered as well, which is implemented with thrust::transform.
 * If no vendor function overload is registered for the given queue or device
 * specification, a generic fallback implementation using alpaka kernels is
 * called.
 */
...
auto validateResult(auto const &input0, auto const &input1, auto const &output)
    -> int {
  using Data = uint32_t;
  ...
  for (std::size_t i(0u); i < output.getExtents(); ++i) {
    ...
    if (val != correctResult) {
      if (falseResults < MAX_PRINT_FALSE_RESULTS)
        std::cerr << "C[" << i << "] == " << val << " != " << correctResult
                  << std::endl;
      ++falseResults;
    }
  }
...
auto example(auto const deviceSpec, auto const exec, size_t numElements)
    -> int {
  ...
  vendorExample::Transform::call(queue, output_d, std::plus{}, input0_d,
                                 input1_d);
  ...
  if (int const result = validateResult(iotaBuffer_h, iotaBuffer_h, output_h);
      result != EXIT_SUCCESS)
    return result;
  ...
  return onHost::executeForEach(
      [=](alpaka::concepts::BackendSpec auto const &backend) {
        ...
        return example(alpaka::onHost::DeviceSpec{backend},
                       alpaka::getExecutor(backend), numElements);
      },
