...
int example(int argc, char *argv[], auto const usedApi) {
  ...
  // try to load first in working directory, then in binary directory, then via
  // LD_LIBRARY_PATH
  ...
  for (auto libPrefix : prefixList) {
    ...
    handle = dlopen(toLower(libraryName).c_str(),
  ...
  if (handle) {
...
#if ALPAKA_OS_WINDOWS
    fn_startBackend =
        (startBackend_t)GetProcAddress(libBackend, "xmrstak_start_backend");
    if (!func) {
    ...
    auto func = reinterpret_cast<alpaka_main_t *>(
        dlsym(handle,
              "alpaka_main") // <-- if compiled without extern "C", use mangled
    );
    if (!func) {
...
void help(char *argv[]) { std::cerr << argv[0] << " [-h]" << std::endl; }
...
auto main(int argc, char *argv[]) -> int {
  ...
  return onHost::executeForEach(
      [=](auto const &api) { return example(argc, argv, api); }, onHost::apis);
}
