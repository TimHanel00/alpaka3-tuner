...
void forEachDeviceType(auto const deviceSpec) {
  ...
  std::cout << "\tdevice kind      : "
            << onHost::getName(deviceSpec.getDeviceKind()) << "\n";
  std::cout << "\tnumber of devices: " << numDevices << "\n";
  for (uint32_t d = 0; d < numDevices; ++d) {
    onHost::DeviceProperties devProps =
        computeDevSelector.getDeviceProperties(d);
    std::cout << "\t\tid                   : " << d << "\n";
    ...
    std::cout << "\t\tglobal memory bytes  : "
              << devProps.globalMemCapacityBytes << "\n";
    auto executors =
        onHost::getExecutorsList(deviceSpec, exec::enabledExecutors);
    std::cout << "\t\texecutors            : ";
    std::apply(
        [](auto... executors) {
          bool first = true;
          ((std::cout << (first ? "" : ", ") << onHost::getName(executors),
            first = false),
           ...);
        },
        executors);
...
extern "C" int alpaka_main(int argc, char *argv[]) {
  ...
  std::apply([](auto... devSpec) { (forEachDeviceType(devSpec), ...); },
             deviceSpecs);
