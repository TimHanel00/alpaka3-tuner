 ...
 * algorithm is based on Blelloch, written up in the CUDA blog:
...
public:
  ALPAKA_FN_ACC void operator()(auto const &acc,
    ...
    concepts::CVector auto largeChunkExtents =
        CVec<IdxType, elsPerThread * chunkExtents.x()>{};
    ...
    for (auto chunkIdx :
      ...
      auto tmp = onAcc::declareSharedMdArray<Data, uniqueId()>(
          acc, CVec<IdxType,
                    conflictFreeAccess<DeviceType>(largeChunkExtents.x())>{});
      auto const chunkOffset = largeChunkExtents * chunkIdx;

      // -- COPY TO SHARED MEM --
      for (auto idxInChunk :
           onAcc::makeIdxMap(acc, onAcc::worker::threadsInBlock,
                             IdxRange{largeChunkExtents})) {
        if (chunkOffset + idxInChunk < numElements)
          tmp[conflictFreeAccess<DeviceType>(idxInChunk)] =
              inputVec[chunkOffset + idxInChunk];
        else
          tmp[conflictFreeAccess<DeviceType>(idxInChunk)] = 0;
      }

      // -- UP-SWEEP / REDUCE --
      for (IdxType d = largeChunkExtents.x() / 2_idx, offset = 1_idx; d > 0;
           d >>= 1, offset <<= 1) {
      ...
      for ([[maybe_unused]] auto idxInChunk :
        ...
        if constexpr (sizeof...(blockSums)) {
          ...
          _blockSums[chunkIdx] =
              tmp[conflictFreeAccess<DeviceType>(largeChunkExtents - 1_idx)];
        }
      ...
      // -- DOWN-SWEEP --
      for (IdxType d = 1, offset = largeChunkExtents.x() / 2_idx;
           d < largeChunkExtents; d <<= 1, offset >>= 1) {
        onAcc::syncBlockThreads(acc);
        for (auto idxInChunk :
             onAcc::makeIdxMap(acc, onAcc::worker::threadsInBlock,
                               IdxRange{CVec<IdxType, 0>{},
...
void scan(auto &exec, auto &devAcc, auto &queue,
  ...
  if (frameSpec.getNumFrames() > 1_idx) {
    ...
    auto addIncrementsFrameSpec = onHost::FrameSpec{
        ...
        CVec<IdxType, largeChunkExtents.x()>{}, exec};
    using AddLaunchVec = typename std::remove_cvref_t<ALPAKA_TYPEOF(
        addIncrementsFrameSpec.getNumFrames())>::UniVec;
    auto const addTunables = alpakaTune::TunableBundle{
        alpakaTune::tuneNumFrames(
            addIncrementsFrameSpec,
            alpakaTune::generate::linSpace(
                AddLaunchVec::fill(1u),
                AddLaunchVec{addIncrementsFrameSpec.getNumFrames()},
                AddLaunchVec::fill(1u))),
        alpakaTune::tuneFrameExtent(
            addIncrementsFrameSpec,
            alpakaTune::generate::linSpace(AddLaunchVec::fill(1u),
                                           AddLaunchVec::fill(512u),
                                           AddLaunchVec::fill(1u)))};
    auto addTuning = alpakaTune::makeTuner(addTunables, devAcc,
                                           addIncrementsFrameSpec.getExecutor(),
                                           "scan/increments");
    // enqueue the kernel execution tasks
    auto const scanBundle =
        KernelBundle{scanBlocks, numLargeChunks, chunkExtent,
    ...
    auto const addBundle =
        KernelBundle{addIncrements, largeChunkExtents, blockSums, outputVec};
    std::size_t addExecutions = 0u;
    while (alpakaTune::example::applicationRunsRemain(addExecutions, scanTuning,
                                                      addTuning)) {
      scanTuning.enqueue(queue, frameSpec, scanBundle);
      addTuning.enqueue(queue, addIncrementsFrameSpec, addBundle);
      ++addExecutions;
    }

    // need to wait here until the previous call is done before we can destruct
    // the buffers for increments/blockSums when running out of scope
    onHost::wait(queue);
  } else {
    // problem fits within 1 frame
    auto const scanBundle = KernelBundle{scanBlocks, numLargeChunks,
                                         chunkExtent, inputVec, outputVec};
    std::size_t scanExecutions = 0u;
    while (alpakaTune::example::applicationRunsRemain(scanExecutions,
                                                      scanTuning)) {
      scanTuning.enqueue(queue, frameSpec, scanBundle);
