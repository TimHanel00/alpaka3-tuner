 ...
 *
 * This file contains the main setup and surrounding functions for the scan
 * example.
 */
...
void printExampleHeader(ScanType const scanType, IdxType const numElements,
  ...
  if (enableInPlace)
    std::cout << "    Running in-place (input buffer = output buffer)"
              << std::endl;
  else
    std::cout << "    Not running in-place (input buffer != output buffer)"
              << std::endl;
...
auto example(auto const deviceSpec, auto const exec, IdxType numElements,
             bool enableStdScan, bool enableCheck, bool enableInPlace,
  ...
  return runExample(exec, devAcc, queue, inputData, bufAccX, bufAccY,
                    numElements, scanType, enableCheck, enableStdScan);
}
...
void help(char *argv[]) {
  ...
  std::cerr << "  -n  numElements: Number of elements to process. Default: "
               "2^24 = 16 Mi"
            << std::endl;
  std::cerr << "  -e: disable execution of the native std::inclusive_scan or "
               "std::exclusive_scan implementation"
            << std::endl;
  std::cerr << "  -c: disable checking for correct results" << std::endl;
  std::cerr << "  -i: enable in-place scan instead of creating an output "
               "buffer for the result"
            << std::endl;
  std::cerr << "  -t: scanType: 0 for exclusive, 1 for inclusive scan. "
               "Default: 0 (exclusive)"
            << std::endl;
  std::cerr << "  -h: Print this help message" << std::endl;
  std::cerr << std::endl;
  std::cerr
      << "Example call for an in-place inclusive scan with 256Ki elements:"
      << std::endl;
  ...
  auto result = onHost::executeForEach(
      [=](alpaka::concepts::BackendSpec auto const &backend) {
        ...
        return alpaka::example::scan::example(
            ...
            numElements, enableStdScan, enableCheck, enableInPlace, scanType);
      },
