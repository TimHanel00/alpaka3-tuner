...
/* This function introduces padding to the shared memory accesses to reduce bank
 * conflicts between threads. The template parameter is the device kind, which
 * dictates how many memory banks are assumed. For CPU or unknown/unimplemented
 * device kinds, infinite memory banks are assumed, i.e., no padding is used.
 */
 ...
 */
ALPAKA_FN_ACC Data scanMiniBlock(Data *block,
                                 concepts::CVector auto const &extent) {
#if 0
    std::cout << "scanMiniBlock extent: " << extent << " block data: " << block[0] << ", " << block[1] << ", ..."
              << std::endl;
#endif
  // -- UP-SWEEP / REDUCE --
  for (IdxType d = extent.x() / 2_idx, offset = 1_idx; d > 0;
       d >>= 1, offset <<= 1) {
  ...
  // -- DOWN-SWEEP --
  for (IdxType d = 1, offset = extent.x() / 2_idx; d < extent.x();
       d <<= 1, offset >>= 1) {
    for (auto frameElem = 0_idx; frameElem < 2_idx * d; frameElem += 2_idx) {
      IdxType left = offset * (frameElem + 1_idx) - 1_idx;
      IdxType right = offset * (frameElem + 2_idx) - 1_idx;
...
/* Do an add increment on the given miniblock, adding the given blockSum to each
 * element.
 */
ALPAKA_FN_ACC void addIncrements(Data *block, Data const &blockSum,
                                 concepts::CVector auto const &extent) {
...
/* This kernel calculates an exclusive scan for each block indvidually. The
 * algorithm is based on Blelloch, with the improvement from Lichterman, written
 * up in the CUDA blog (see 39.2.5):
 * https://developer.nvidia.com/gpugems/gpugems3/part-vi-gpu-computing/chapter-39-parallel-prefix-sum-scan-cuda
 */
template <ScanType SCAN_TYPE> class Scan_ScanBlocksKernel {
public:
  ALPAKA_FN_ACC void operator()(auto const &acc,
    ...
    constexpr auto elsPerThread =
        largeChunkExtents.x() / numThreadsPerBlock.x();
    concepts::CVector auto chunkExtent =
        CVec<IdxType, elsPerThread * numThreadsPerBlock.x()>{};
    concepts::Vector auto numElements = inputVec.getExtents();

    constexpr auto miniBlockSize =
        std::min(maximumMiniBlockSize<DeviceType>(), elsPerThread);
    ...
    auto const validElementsInLastFrame =
        (numElements - 1_idx) % chunkExtent + 1_idx;
     ...
     */
    for (auto chunkIdx :
         onAcc::makeIdxMap(acc, onAcc::worker::blocksInGrid,
                           IdxRange{Vec<IdxType, 1u>{0}, numChunks})) {
      bool const lastFrameFull = validElementsInLastFrame == chunkExtent;
      bool const isLastFrame = chunkIdx == numChunks - 1_idx;

      // allocate "per-thread" register memory to store all mini blocks of a
      // thread persistently
      LocalArray regMem;

      auto tmp = onAcc::declareSharedMdArray<Data, uniqueId()>(
          acc, CVec<IdxType,
                    conflictFreeAccess<DeviceType>(miniBlocksPerChunk - 1_idx) +
                        1_idx>{});
      auto const frameOffset = chunkExtent * chunkIdx;

      for (auto frameElem :
           onAcc::makeIdxMap(acc, onAcc::worker::threadsInBlock,
                             IdxRange{CVec<IdxType, 0u>{}, chunkExtent,
                                      CVec<IdxType, elsPerThread>{}})) {
        // -- COPY TO SHARED MEM --
        if ((!lastFrameFull && isLastFrame) || elsPerThread % 4_idx != 0_idx) {
          // load into miniblocks buffer, from frameElem to frameElem +
          // elsPerThread
          ...
          MdSpanArray<LocalArray, IdxType, alpaka::Alignment<16>> regMemMd{
              regMem};

          for (auto i = 0_idx; i < elsPerThread; i += 4_idx) {
            auto inputVecView =
                SimdPtr{inputVec, Vec{frameOffset + frameElem + i},
                        Alignment<16>{}, CVec<IdxType, 4>{}};
            auto regView =
                SimdPtr{regMemMd, Vec{i}, Alignment<16>{}, CVec<IdxType, 4>{}};
        ...
        for (auto miniBlockOffset = 0_idx; miniBlockOffset < elsPerThread;
             miniBlockOffset += miniBlockSize) {
          // scan miniblock
          auto miniBlockSum = scanMiniBlock(regMem + miniBlockOffset,
                                            CVec<IdxType, miniBlockSize>{});

          // write miniblock sum into shared memory
          tmp[conflictFreeAccess<DeviceType>((frameElem + miniBlockOffset) /
                                             miniBlockSize)] = miniBlockSum;
        }
      }

      // -- UP-SWEEP / REDUCE --
      for (IdxType d = miniBlocksPerChunk / 2_idx, offset = 1_idx; d > 0;
           d >>= 1, offset <<= 1) {
        onAcc::syncBlockThreads(acc);
        for (auto frameElem :
             onAcc::makeIdxMap(acc, onAcc::worker::threadsInBlock,
                               IdxRange{CVec<IdxType, 0>{},
                                        Vec<IdxType, 1>{2_idx * d}, 2_idx})) {
      ...
      for ([[maybe_unused]] auto frameElem :
        ...
        if constexpr (sizeof...(blockSums)) {
          ...
          _blockSums[chunkIdx] =
              tmp[conflictFreeAccess<DeviceType>(miniBlocksPerChunk - 1_idx)];
        }
      ...
      // -- DOWN-SWEEP --
      for (IdxType d = 1, offset = miniBlocksPerChunk / 2_idx;
           d < miniBlocksPerChunk; d <<= 1, offset >>= 1) {
        onAcc::syncBlockThreads(acc);
        for (auto frameElem :
             onAcc::makeIdxMap(acc, onAcc::worker::threadsInBlock,
                               IdxRange{CVec<IdxType, 0>{},
      ...
      for (auto frameElem :
        ...
        for (auto miniBlockOffset = 0_idx; miniBlockOffset < elsPerThread;
          ...
          addIncrements(regMem + miniBlockOffset, blockSum,
                        CVec<IdxType, miniBlockSize>{});
        }

        if ((!lastFrameFull && isLastFrame) || elsPerThread % 4_idx != 0_idx) {
          // write back to global mem, from frameElem to frameElem +
          // elsPerThread
          for (auto i = 0_idx; i < elsPerThread; ++i) {
            if (frameOffset + frameElem + i < numElements) {
          ...
          for (auto i = 0_idx; i < elsPerThread; i += 4_idx) {
            auto outputVecView =
                SimdPtr{outputVec, Vec{frameOffset + frameElem + i},
                        Alignment<16>{}, CVec<IdxType, 4>{}};
            auto regView =
                SimdPtr{regMemMd, Vec{i}, Alignment<16>{}, CVec<IdxType, 4>{}};
            if constexpr (SCAN_TYPE == EXCLUSIVE_SCAN)
...
/* Add prefix sum from previous blocks (blockSums) to all elements in each
 * block.
 */
...
void scan(auto &exec, auto &devAcc, auto &queue,
          concepts::IDataSource auto const &inputVec,
          concepts::IMdSpan auto outputVec) {
