 ...
 *
 * This file contains implementations for different backends of the scan
 * example, including a standard library serial implementation and cublas.
 */
...
int validateResult(auto queue, auto const &inputData, auto const &bufY,
                   IdxType numElements, ScanType scanType) {
  ...
  std::cout << "    Copied [Gbyte/s]: "
            << (static_cast<double>(numElements * sizeof(Data)) / copyRuntime) /
                   1.e9
            << std::endl;
  ...
  switch (scanType) {
  case EXCLUSIVE_SCAN:
    std::exclusive_scan(inputData.data(), inputData.data() + numElements,
                        groundtruth.data(), 0);
    break;
  case INCLUSIVE_SCAN:
    std::inclusive_scan(inputData.data(), inputData.data() + numElements,
                        groundtruth.data());
    break;
  }
...
void printResults(double time, IdxType numElements) {
  ...
  std::cout << "    Processed [Gbyte/s]: "
            << (static_cast<double>(numElements * sizeof(Data)) / time) / 1.e9
            << std::endl;
}

int runExampleGeneric(auto &exec, auto const &dev, auto const &queue,
                      auto const &inputData, auto &bufX, auto &bufY,
                      IdxType numElements, ScanType scanType,
  ...
  return enableCheck
             ? validateResult(queue, inputData, bufY, numElements, scanType)
             : EXIT_SUCCESS;
}

// overload for generic executors with no special std implementation
int runExample(auto &exec, auto const &dev, auto const &queue,
               auto const &inputData, // host buffer with input data
               auto &bufX, // accelerator input buffer (uninitialized)
               auto &bufY, // accelerator output buffer (may be same as bufX
                           // when running inplace)
               IdxType numElements, ScanType scanType, bool const enableCheck,
               bool const /*enableStd*/) {
  // copy data to accelerator buffer
  alpaka::onHost::memcpy(queue, bufX, inputData);
  alpaka::onHost::wait(queue);

  return runExampleGeneric(exec, dev, queue, inputData, bufX, bufY, numElements,
                           scanType, enableCheck);
}

// overload for CpuSerial running std:: implementations if requested
int runExample(exec::CpuSerial exec, auto const &dev, auto const &queue,
               auto const &inputData, auto &bufX, auto &bufY,
               IdxType numElements, ScanType scanType, bool const enableCheck,
               bool const enableStd) {
  // copy data to accelerator buffer
  ...
  if (enableStd) {
    ...
    switch (scanType) {
    case EXCLUSIVE_SCAN:
      std::exclusive_scan(bufX.data(), bufX.data() + bufX.getExtents().x(),
                          bufY.data(), 0);
      break;
    case INCLUSIVE_SCAN:
      std::inclusive_scan(bufX.data(), bufX.data() + bufX.getExtents().x(),
                          bufY.data());
      break;
    }
  ...
  auto resGeneric = runExampleGeneric(exec, dev, queue, inputData, bufX, bufY,
                                      numElements, scanType, enableCheck);
...
int runExample(exec::GpuCuda exec, auto const &dev, auto const &queue,
               auto const &inputData, auto &bufX, auto &bufY,
               IdxType numElements, ScanType scanType, bool const enableCheck,
               bool const enableStd) {
  // copy data to accelerator buffer
  ...
  if (enableStd) {
    ...
    // in order to be fair to the alpaka implementation, which allocates its
    // temporary memory inside the measured time too, we need to do the same for
    // the cub implementation
    ...
    case EXCLUSIVE_SCAN:
      ...
      CUDA_CHECK(cub::DeviceScan::ExclusiveSum(
          d_temp_storage, temp_storage_bytes, d_in, d_out, numElements,
...
hipcub::CachingDeviceAllocator
    g_allocator; // Caching allocator for device memory
...
int runExample(exec::GpuHip exec, auto const &dev, auto const &queue,
               auto const &inputData, auto &bufX, auto &bufY,
               IdxType numElements, ScanType scanType, bool const enableCheck,
               bool const enableStd) {
  // copy data to accelerator buffer
