...
int example(auto const devSpec) {
...

  {
    ...
    std::cout << "memory buffer on "
              << alpaka::onHost::getStaticName(alpaka::getApi(device_buffer))
              << " at " << std::data(device_buffer) << "\n\n";

    // once the device buffer goes out of scope, it will enqueue its free, so
    // all other operations have finished before the memory is freed
...
auto main() -> int {
   ...
   *
   * If you would like to execute it for a single device only you can use the
   * following code.
   *  @code{.cpp}
