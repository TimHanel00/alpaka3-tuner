...
auto main() -> int {
  ...
  /** This call is showing what is hidden in the generator
   * onHost::getDeviceSpecsFor later on used.
   *
   ...
   *
   * If you change this example to `onHost::DeviceSpec{api::cuda,
   * deviceKind::nvidiaGpu}` and the CMake option `alpaka_DEP_CUDA` is set to
   * `OFF` compile will fail.
   */
  example(onHost::DeviceSpec{api::host, deviceKind::cpu});

  /** Execute the example once for each enabled API and device kind including
   * `onHost::DeviceSpec{api::host, deviceKind::cpu}` explicitly called
   * before.*/
