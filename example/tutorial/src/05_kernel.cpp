...
struct VectorAddKernel {
  ...
  operator()(TAcc const &acc, alpaka::concepts::IDataSource auto const &in1,
             ...
             alpaka::concepts::IMdSpan auto out, uint32_t size) const {
    for (auto [index] :
...
void testVectorAddKernel(alpaka::onHost::concepts::Device auto device,
  ...
  std::cout << "Testing VectorAddKernel with scalar indices with a grid of "
            << frameSpec << "\n";
  auto const scalarBundle =
      alpaka::KernelBundle{VectorAddKernel{}, in1_d, in2_d, out_d, size};
  while (!scalarTuning.isTuningComplete())
  ...
  std::cout << "Testing VectorAddKernel with vector indices with a grid of "
            << frameSpec << "\n";
  auto const vectorBundle = alpaka::KernelBundle{VectorAddKernel1D{}, in1_d,
                                                 in2_d, out_d, Vec1D{size}};
  while (!vectorTuning.isTuningComplete())
...
auto main(int argc, char **argv) -> int {
  ...
  /* This example requires additionally to the device specification an executor
   * to describe how the parallelism on the compute device is organised. If you
   * would like to execute it for a single accelerator only you can use the
   * following code.
   *  @code{.cpp}
