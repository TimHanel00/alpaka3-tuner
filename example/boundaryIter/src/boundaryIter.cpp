...
/** @brief An example boundary kernel that copies only the values in the given
 * boundary direction
 */
struct BoundaryExampleKernel {
  ALPAKA_FN_ACC auto
  operator()(auto const &acc, concepts::IMdSpan auto const &view,
             concepts::IMdSpan auto viewTarget,
             concepts::BoundaryDirection auto const &bd) const {
    for (auto idx : onAcc::makeIdxMap(acc, alpaka::onAcc::worker::threadsInGrid,
                                      IdxRange{view.getExtents()}, bd)) {
...
auto printBoundaryContainer(auto view, auto bd_container) {
  std::cout << "Number of unique boundaries: " << bd_container.length()
            << std::endl;
...
int example(auto const devSpec, auto const computeExec) {
  ...
  if constexpr (std::is_same_v<ALPAKA_TYPEOF(device),
                               onHost::Device<api::Host, deviceKind::Cpu>>) {
    for (auto &el : view)
      std::cout << el << " ";
    std::cout << std::endl;
  }
  ...
  auto frameSpec =
      onHost::FrameSpec{divCeil(size, chunkSize), chunkSize, computeExec};
  using NumFrames = typename std::remove_cvref_t<ALPAKA_TYPEOF(
  ...
  {
    std::cout << "Halo size 1 (default), constructed directly from the view"
              << std::endl;
...

  {
    ...
    constexpr auto bdContainerHaloHeterogeneous =
        makeBoundaryDirIterator(lowerHalo, upperHalo);
