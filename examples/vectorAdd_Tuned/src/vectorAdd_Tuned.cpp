/* Copyright 2024-2025  Jan Stephan, Luca Ferragina, Aurora Perego, Andrea
 * Bocci, René Widera, Mehmet Yusufoglu, Tim Hanel SPDX-License-Identifier:
 * MPL-2.0
 */
...
auto validateResult(auto const &bufHostC, auto const &bufHostA,
                    auto const &bufHostB, std::size_t extent) -> int {
  using Data = uint32_t;
...
template <typename SimdConcurrency> class VectorAddTunedKernel {
public:
  ...
  ALPAKA_FN_ACC auto operator()(auto const &acc,
    ...
    simdGrid.concurrent<SimdConcurrency::value>(
        acc, numElements,
...
auto example(auto const deviceSpec, auto const exec, size_t numElements,
  ...
  // how many elements one worker should compute to ensure vectorization or
                    ...
                    .template withConstraint<aTune::frame::numThreads,
                                               ...
                                               present.*/
                        {
                          bool blocksHigherOneIfPossible =
                              (dataBlocking.m_numFrames.x() == 1) ||
                              (numBlocks.product() > 1);
                          return blocksHigherOneIfPossible &&
                                 numThreads.x() % 32 ==
                                     0; /* based on our definition this is
            always true -- only showcase constraint definition */
                        })
                    // define a constraint for the compile time Tunable
                    .template withConstraint<
                        compileTimeTunableID>([&](auto const
                                                      &
                                                          compileTimeTunable /*compileTimeTunables are exposed as std::variants in the constraint*/) {
                      return std::visit(
                          [](auto const &val) {
                            // val is the *active type* inside the variant
                            return (val & (val - 1)) == 0;
                          },
                          compileTimeTunable);
                    })
...
auto main(int argc, char *argv[]) -> int {
  ...
  return onHost::executeForEachIfHasDevice(
      [=](auto const &backend) {
        return example(backend[alpaka::object::deviceSpec],
